...
This tutorial walks through the creation of a reproducible document that can be reproduced automatically.
Though reproducibility is a small part of the research process, it can serve as the mortar of open science building blocks (e.g. preregistration, open data, postpublication review) --- but not that even non-open science will benefit from ensuring reproducibility because good scientific practice demands that every scientist is able to reproduce their own results later in time.
...
A step to prevent unpleasant surprises is to simulate data.<!-- AP: this paragraph deserves some references. I assume those can be found in the Bayesian literature. But are there classic SEM literature references talking of the value of simulation. Please make a lit research -->
...
The selected model parameters should express our expectations for the Machiavellism subscale of the short dark triad (SD3). <!-- AP: reference needed; explain the scale. What is the construct that we are measuring here? -->
As a starting point, we can use already published information.
@Jones2013 report a set of factor loadings (see Table \@ref(tab:loadings)) <!--AP what was the reference population? what was the sample size? --> and report a gender difference (at the manifest level) depending on the sample between `r min(cohend_jones_paulhus)` and `r max(cohend_jones_paulhus)`. <!--AP why not on the latent level? They report factor loadings after all? Please explain-->
...
Data simulation is particularly easy in lavaan syntax using the function `simulateData`. More complicated simulations (for example, including missing data simulation) are possible with other packages, e.g., simsem <!--AP please add reference, and possibly other packages-->. The following code simulates data from the population model `model_truth` with 1.000 cases in each group. 
...
It is a little more complicated to achieve uncorrelated random numbers in parallelized code (e.g., when the same code is run with different portions of data in a computer cluster), but the `future` <!--AP add citation --> package handles this for us:
<!--AB I don't understand how this solves the problem yet - please explain in more detail -->
```{r}
set.seed(1235)
runif(1)
set.seed(1235)
...
Historically\footnote{Apparently, the use of readme files can be traced back to at least 1974 <!--AP reference needed, I guess-->}, a readme file is the default place to look for human readable information in a computer archive 
...
Note that publishing data, code, or other materials is not sufficient to allow other users to use it, modify it, or redistribute it. For example, according to German copyright law, a repository without a license must be assumed to be protected by copyright <!-- AP citation needed -->. In many countries (i.e. the USA, European Union), a content creator has exclusive right to use their creation.
...
It does include Ubuntu, R, RStudio, LaTeX as well as many R packages like the tidyverse <!-- AP add citations to all packages, please--> (amounting to `r installed.packages() %>% as.data.frame() %>% filter(is.na(Priority)) %>% nrow()` packages.)
...
## Creating a DOI
<!-- AP add references; why are DOIs helpful-->
...
```{r, include=FALSE}
knitr::write_bib(c(.packages(), "gert", "bookdown", "lavaan", "knitr", "targets", "renv"),
                 "packages.bib")
```
