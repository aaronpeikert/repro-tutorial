...
[^popper]: In fact, @popperLogicScientificDiscovery2002's famous criterion of demarcation is built around "inter-subjective testing", a concept which he later generalised to inter-subjective criticism (Footnote *¹, p. 22). Somewhat confusingly, @popperLogicScientificDiscovery2002 uses the term "Reproducibility" for what we would call "Replication".
...
Version control allows a clear link between which results were generated by which verion of code and data.
...
A version controlled project is makes the loss of files unlikely and also supports remote collaboration.

We recommend to use Git for version control, because of its widespread adoption in the R community.
Git is build around snapshots which represent the entire project state at a given point in time.
...
Each commit "knows" its ancestor and they build a timeline together.
The conglomerate of commits (i.e. the version controlled project) is called repository.
...
To collaborate on a Git repository, the repository has to be uploaded somewhere.
...
However, while using version control and dynamic document generation are becoming more common @ argue in most instances this approach fails to garuntee reproducibility.
...
Similar to a collection of cooking recipes, we can have multiple products (_targets_) with different ingredients (_requirements_) and different steps of preparation (_recipe_).
...
We recommend to use Make for dependency tracking because it is language independent.
...
One solution to problem of external software dependencies is to provide not only the analysis script but all dependent software packages and system libraries in a so-called _container_.
...
```{r, include=FALSE}
knitr::write_bib(c(.packages(), "repro", "here", "rticles", "gert", "bookdown", "lavaan", "knitr", "targets", "renv", "tidyverse"),
                 "packages.bib")
```
