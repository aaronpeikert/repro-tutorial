...
With regard to the first goal, we discuss how to combat four common threats to reproducibility with tools originating from software engineering [see @Peikert2019 for details].
Then we present a tutorial on how to employ these tools to achieve automated reproducibility.
This tutorial can not introduce the reader comprehensively into each tool but intents to help readers who have picked up bits and pieces to build a coherent workflow.
...
Ideally, our reader already has created a dynamic document at some point (e.g. with RMarkdown or Jupyter) and has tried their hand once at version control (e.g. with Git). 
...
If, due to whatever reason, some parts of the workflow proof to be unfeasible, we encourage the reader to find their own suitable solutions and to concentrate on the feasible parts.
...
Another property of a computational recipe is that the same inputs of should always result in the same outputs.
Fo most computer code (given the same software is used) this property is fulfilled, however, one noteworthy exception are pseudo random numbers.
...
To ensure identical random numbers, users may fix the state of the random number generate with a so called seed (e.g. in R with `set.seed()`).
...
Users can therefore quickly change the software environment e.g. update to another R or package version without being "stuck" with an `Docker` image they do not know how to rebuild.
...
Unfortunately, Docker requires administrative rights to run.
We recommend [`renv`](https://rstudio.github.io/renv/articles/renv.html) [@R-renv] in cases where no administrative rights can be obtained but can not detail its use in this document.
...
If you can not share your data publicly add it to the `.gitignore` file and share it on request.
...
Noteworthy is the package output format supplied by the [`papaja`](https://github.com/crsh/papaja) package [@papaja] which produces APA compliant manuscripts [@apa7].
...
Even when the researcher was faithful in each aspect to the preregistration the reader may reach opposite conclusions, because the reader has to compare two different texts which may be worded differently or describe the same thing in varying level of detail.
...
Reproducibility is of utmost importance at this stage since the preregistration must produce valid results at two points in time.
We can only recommend PAC when the dangers to reproducibility we described above are eliminated and a state of automated reproducibility is reached.
...
One factor that reduces comprehensiveness of such manuscript style but that is absent in standard preregistrations are strict word count limits that some journals impose.
In such case we suggest using appendix materials to keep the manuscript concise, yet the preregistration comprehensive.
...
For each study these tasks have to specifically coded.
...
In order to do so researcher must walk a fine but utmost important line.
Of course researcher must not analyze the data before they write the preregistration.
Nevertheless, they may obfuscate the real data by shuffling all variables columnwise (independently from each other) or otherwise create synthetic data that allows them to test the function `planned_analysis()` without actually revealing the research outcomes.
Preregistration after data collection is common when reanalyzing data from another research group [@westonRecommendationsIncreasingTransparency2019] but not so much within the same research project.
We argue that it is still an eligible preregistration.
Since the data is obfuscated, almost no results are revealed and the researchers can not employ their researcher degrees of freedom effectively to influence the results.
...
To be clear, any exploration of the data before preregistration renders the preregistration invalid and amounts to grave scientific (albeit sometime unintentional) misconduct.
...
However, we must warn our readers, while piloting is more traditional than our approach of obfuscating the data before preregistration it may not be innocuous to use the resulting data and care must be taken to avoid errors of sequential testing [@whiteheadBiasMaximumLikelihood1986].
...
One significant difference is that PAC demands a comprehensive analysis plan.
With traditional preregistration, however, the researcher gets away with deciding details later.
Some researchers may not feel ready to decide every detail.
In such case, we recommend to decide based on the best available knowledge and if these decisions prove infeasible, revise and explain them later as described in the section [Deviating from the preregistration and exploration].

Two applications, however, lend themselves to PAC.
First, registrations of clinical trials typically describe analyses in excruciating detail.
Those registrations typically contain a detailed description of how results will be presented, including example tables and graphics.
...
Second, preregistering explorative statistical models (i.e. machine learning) is hardly feasible with standard preregistrations since they are too complex to describe and depend strongly on their software implementation verbally.
PAC, however, captures the precise algorithmic model, including its software implementation and is ideal for preregistering these models.
...
To organize studies with multi hypotheses we suggest multiple (possibly numbered in accord with the hypotheses, e.g. 1.2, 2.3 etc.) `planned_analysis()` and `report_analysis()` functions.
Steps that apply to multiple analyses should be extracted into their own function e.g. `discard_outliers()` or `extract_scores()`.
Preregistrations that cover multiple distinct data sources may employ multiple `simulate_data()` functions.
These are merely suggestions, researcher who are more comfortable with another programming style will surely find an appropriate system.
...
After the researcher is satisfied with their draft preregistration, they should create an archived and unchangeable version of the project that serves as the preregistration.
...
The necessary steps for archival on zenodo.org are described in at the end of section [Preregistration as Code --- a Tutorial].
