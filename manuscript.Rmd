...
It serves to separate a-priori planned and theory-driven (confirmatory) analyses from unplanned and post-hoc (exploratory) analyses.<!-- CJ I removed the bit about credibility, because that requiresfurther explanation of why separating these two types results in more credible findings-->
...
One is that written study plans are often multiinterpretable.
...
Moreover, it reduces overhead by obviating the need to write a separate preregistration and manuscript.
...
As outlined before, reproducibility builds upon four pillars (version control, dynamic document generation, dependency tracking, and software management).<!-- CJ I don't understand why you repeat this here. Remove?-->
To use PAC the dangers to reproducibility we described must be  eliminated.<!-- CJ I don't understand why you repeat this here. Remove?-->
...
We believe that pairing PAC with the workflow presented above offers several advantages over classical preregistration.
...
Fourth, despite its rigor, PAC accommodates partly exploratory data-dependent decisions.
Some decisions can be anticipated and formulated as conditional code.
...
Some necessary deviations from the preregistration only become apparent after seeing the data.<!--CJ I merged point 5 here; it's part of point 4-->
Version control makes it possible to commit specific changes to the planned analyses.
It is recommended to documented all such changes by maintaining a changelog, and referencing specific commits.<!--CJ I do this myself, I think it's a good idea.-->
...
With PAC researchers can precisely calibrate how much they want to explore.<!--CJ This is vague-->
When communicating a finding as explorative, it may still make a difference if tens or hundreds of variables were considered.<!-- CJ Not clear what this refers to; you haven't given an example or anything! Cut?-->
...
Transparency is essential in this regard <!--CJ: it is not clear how the following argument justifies the need for transparency. Please fix--> because an empirical test of a theory can not test the theory alone but tests it in conjunction with auxiliary assumptions [@meehlTheoreticalRisksTabular1978].
...
When evaluating the planned analyses on the empirical data, these two covariates reveal high collinearity, thus biasing estimates and compromising statistical inference.
...
This change to the preregistration directly relates their theory of intelligence.
...
<!--CJ not sure what this final line intended to argue. I removed it. Feel free to add a more satisfactory conclusion to this paragraph-->
...
The second kind involves rigorous exploration using methods that are standard in machine learning [@brandmaier22mlsem].
These models often involve dozens, if not hundreds of predictors, which makes it difficult to describe them verbally.
With PAC, such models can be preregistered clearly and in comprehensive detail.
...
a piece of information that is crucial to assess e.g., whether the results might be overfit <!--CJ reference Hastie & Tibshirani-->.

## How to Write a Good PAC

When planning a study and writing a PAC, researchers ought to strive to meet three goals:<!--CJ: None of these goals relate explicitly to code. Is that a problem? Also: Shouldn't you mention the goals when you first introduce PAC?-->
...
For example, the APA standards [@apa7] require research articles describe a study in such detail that any researcher that is familiar with the field is able to repeat it.
These well-known standards thus provide a good starting point for the structure of a PAC.<!--CJ What are you arguing here? You've just convinced the reader that existing templates are not comprehensive enough. Now you're arguing that APA7 can be considered a preregistration template, and that it is comprehensive? I am a little confused here--> 
One particular advantage is introduction's and theoretical background's embedding in existing literature where that provide the basis for the study's design.<!--CJ This sentence has been missed by the language editor-->
Further inspiration may be taken from preregistration templates relevant to the type of study at hand,
because such templates address methodological rigor more comprehensively.
One factor that limits the comprehensiveness of published scientific reports is the word count imposed by journals.
If this is a concern, we suggest relegating the PAC to a reproducible appendix to keep the core manuscript concise.

Simulating data offers further benefits in ensuring the comprehensiveness of a preregistration.
It forces the researcher to consider conceptual data-analytic challenges before obtaining empirical data [@axelrodAdvancingArtSimulation1997].
It also allows researchers to verify that planned analyses are feasible and likely to run without failure [@braiekTestingMachineLearning2020] (e.g., does the code also work with missing values, extreme outliers, etc.).
Many errors and omissions are likely to come to light when evaluating the intended analysis code on simulated data.

One final method to ensure comprehensivenes of the PAC is peer review.
The value of PAC would be further enhanced by submitting it as a registered report,
a format in which articles are accepted for publication "in principle" based on peer review of the preregistered study plans [@nosekRegisteredReports2014; @chambersWhatNextRegistered2019].
...
Moreover, reviewers can suggest specific improvements to the planned analysis code.

### Effectiveness

If the comprehensiveness criterion is met, researchers can further check whether the planned analyses are effective.
Effectiveness can be defined in terms of ability to recover true population parameters, or power to detect a population effect.<!--CJ You have not said anything about a simulation yet, so this is kind of surprising-->.
Effectiveness can be assessed by means of a simulation study.
Once code has been written to simulate synthetic data, it is often a minor step to perform a simulation study.
It is sufficient to repeat the planned analyses many times, and plot the distribution of estimated parameters, or tabulate the proportion of replications in which a true effect was detected (power analysis).
The R command `replicate()` can be used to this end.
If an analysis fails to consistently recover the population parameters under simulated (idealized) conditions,
it would be unreasonable to expect this analysis to be effective under realistic conditions.
Because the simulation describes the researchers' expectations about real-world conditions,
it is the best way to assess the effectiveness of the analysis prior to collecting empirical data.
Simulating data requires researchers to explicate their assumptions about the data-generating mechanism in concrete computational terms; e.g. assumptions about the distributions of variables and how they relate to one another.
If a researcher experiences difficulty in specifying such assumptions, this may indicate that further literature research or exploratory pilot studies are required.

### Efficiency<!-- CJ I do not understand how 'efficiency' is different from effectiveness above. You cannot assess effectiveness by means of a single iteration. Moreover, efficiency has a specific statistical definition (pertaining to the amount of information required to estimate a parameter). Using this term can be highly confusing to our (probably statistically inclined) readership.-->
...
simulate_data <- function(n, effect_size){
  ...
  x <- rnorm(n)
  # Calculate dependent variable Y as a function of population effect size and residual error
...
Then, using the simulated dataset as a placeholder, one can write a scientific report including all analysis code blocks and the presentation of results.
