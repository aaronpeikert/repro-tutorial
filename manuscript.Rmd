...
A cornerstone of scientific progress is the possibility to critically assess the validity of peer scientists’ scientific conclusions. <!-- CJ More fundamental: Reproducibility is a cornerstone of the scientific METHOD. I think it's more important to emphasize this, rather than critical assessment of others' work. See Karl Popper, The Logic of Scientific Discovery, Routledge, London, 1992, p. 66. --> 
Scientific claims should not be credible because of their originators' authority but by the transparency and replicability of their supporting evidence [@aac4716]. <!-- CJ This is a red herring; nobody said that an argument to authority should be the foundation of scientific evidence. I would suggest focusing on replicability as an essential component of the scientific method. -->
...
Even worse, researchers might often not even be able to achieve reproducibility---defined as the ability to obtain identical results from the *same* data with the *same* computer code. <!-- CJ I would introduce the topic of reproducibility differently. "Even worse" is editorializing; not appropriate for a scientific paper. Instead, consider saying: More fundamental even than replication across datasets is the reproducibility of findings within a single dataset. --> 
When analyses are not properly documented, small mistakes, or circumstances outside the researcher's control (e.g., a corrupted file, a coauthor leaving academia) may render them non-reproducible. 
Over the past decades, computational statistical methods have radically changed the ability of researchers from all disciplines to process and analyze data with complex statistical models. <!-- CJ I think it's more important to focus on the methods that have been developed to ensure reproducibility!)-->
...
With computationally intensive data analyses <!-- CJ The complexity of the analyses is orthogonal to the necessity to make them reproducible. -->, access to the steps taken to process data and generate findings is as important as access to the data themselves [@vanlissa2020worcs].
Only with access to both data and procedures <!-- CJ and software environment --> can we guarantee full computational reproducibility, meaning that the creations emanating from a given data analysis (a statistical result or an entire manuscript describing the results) can be exactly reproduced at any time in the future, by any person, and on any computer.<!-- CJ this final claim is questionable, at least before introducing the concept of containerization. Just using data and procedures, you cannot guarantee reproducibility on any computer.-->

In fact, the research community has long accepted reproducibility as an important element of "good scientific practice." <!-- CJ: This is the crucial point. I would start the paper with this. See my reference to Popper in the opening sentence. -->
However, ensuring reproducibility of computational analyses is still tedious and difficult <!-- CJ I would not editorialize this. Moreover, this sounds discouraging. I would focus on what you have done to make reproducibility more attainable. -->, and there are still no widely accepted standards that can guide researchers when setting up their data analyses. <!-- CJ These are two separate points, each important enough to have their own sentence, with some further clarification. --> 
Even though transparency has increased across scholarly disciplines (that is, data and methods are increasingly openly shared), most of these open repositories <!-- CJ you have not introduced any open repositories yet. --> do not provide sufficient information to reproduce relevant computational and statistical results.
For example, @hardwicke2018 attempted to replicate open materials in the journal Cognition. <!-- CJ How do you replicate open materials? At least in the social sciences "open materials" is taken to mean that you have used a permissive license on a newly developed questionnaire or other research material. -->
...
Thus, there is a dire need to establish standards for reproducible workflows for scientific rigor.<!-- CJ It does not automatically follow that the problem you describe necessitates reproducible workflows. I would procede as follows: 1) use this example to conclude the existence of a problem (even "open" studies do not reproduce), 2) present workflows as a solution. -->

From our personal experience,<!--CJ This undermines the credibility of the argument. This is not an opinion piece--> the lack of a proper incentive structure and the dreary perspective of having to spend several extra hours if not days to properly set of a code repository to become reproducible, makes many scholars skip this extra step. <!-- CJ I am not sure this is the primary reason. Indeed, incentives are important - but in that context, perhaps reference DORA? I think you were just on a good track, arguing that even with the best intentions, researchers often fail to make their work reproducible. Your workflow will ensure that their next attempt is succesful.-->
If reproduction would require merely minutes instead of hours,<!-- CJ Now we're back to ease-of-use, not incentives.-->
it could greatly facilitate collaboration within and across research projects. <!-- CJ This is a different, third argument: In order to collaborate effectively, researchers should be looking at exactly the same data, code and results. I suggest to organize these arguments, and address them each exhaustively in turn. --> 
<!-- Guiding principle: spending machine compute time instead of human research time -->
To achieve this boost in productivity, we must place the burden of reproducibility upon computers instead of the researchers.<!-- CJ I understand what you mean, but it's not yet communicated effectively because you have not yet made explicit that reproducible projects will indeed facilitate collaboration, and increase opportunities for collaboration (e.g., with strangers through github), and will therefore lead to a boost in productivity. Also - ask yourself if this boost in productivity is convincing to individual researchers, your target audience. It seems like this would be more convincing to management and policy makers.-->
To this end, we demand <!-- CJ It's you creating the product, so.. you demand this of yourself? Or do you mean that you demand this of your readership? Either way, strange choice of words.-->
that a scientific product (no matter whether this is a preliminary report or a full research article) should both be understandable by researchers and automatically reproducible by computers with minimal constraints to the researchers' own workflows. <!-- CJ This is ambiguous. The point of "understandable by researchers and automatically reproducible by computers" requires clarification. "Minimal constraints to the workflow" is also not clear. Aren't you providing readers with a workflow? -->
...
We identified the following common four causes for non-reproducibility: <!-- CJ How did you identify these causes? Is this exhaustive?-->
...
This tutorial follows the recommendations of @Peikert2019 and uses _Git_ for version control, _R Markdown_ for dynamic document creation, _Make_ for dependency tracking, and _Docker_ for software management. <!-- CJ None of these terms have been defined yet-->
The interplay of these four components is shown in Figure \@ref(fig:schematic).
However, which tools you use to implement these concepts is a matter of taste and project requirements, and other choices are possible depending on the context and the degree of reproducibility one aims for [e.g., @vanlissa2020worcs; @turingway2021turing].<!-- CJ But readers do not yet know what these concepts are, nor what tools exist. At this point in the writing, I think you need to take them by the hand and explain. After doing that, you can let them know that there are alternatives.-->
...
This paper was designed to help researchers to practically apply the TOP guidelines [except for number 8. which does not apply to single research projects, @nosekPromotingOpenResearch2015; @vanlissa2020worcs].<!-- CJ Is this really the case? Seems like there is little attention to the TOP guidelines, and the present paper is much more widely applicable. Maybe make the general point that workflows can help address formal guidelines for transparency, like TOP and FAIR - and consider citing WORCS for more information. Then explain that your workflow is flexible enough to go beyond--> 
We start by sketching an initial idea in a repository shared with colleagues that potentially collaborate only remotely.<!-- CJ Is this the description of the workflow?-->
...
Nevertheless, everything else is kept as close to a real research question as possible and we have preregistered the analysis without looking at the data beforehand and use actual data subsequently.

Here, we present a workflow that has proven useful for our way of working with a strong focus on Open Science.<!-- CJ What do you mean here? -->
...
This tutorial is targeted at scholars with at least intermediate R knowledge<!--CJ This sounds like something that belongs in the beginning of the introduction; it's not part of setup--> and we assume that you have already installed R and RStudio (if not, check the `r link("https://github.com/aaronpeikert/repro-tutorial/blob/main/install.md", "installation guide")`).
To create reproducible research projects in R, we have implemented our workflow in the  [`repro`-package](https://github.com/aaronpeikert/repro)[^repropackage]: <!-- CJ This also seems like something that should be mentioned in the intro; once people are setting up their systems, they should havebeen told what they're setting them up for-->
...
remotes::install_github("aaronpeikert/repro") # CJ Suggest publishing (at least aminimal working version) of the repro package to CRAN before publishing this paper, to reduce one layer of complexity for users
library("repro")
```
...
For example, the package can check if your computer has all the required software packages installed and ready. <!-- CJ Does repro assume that these have already been installed  by hand? -->
...
If any of these commands find that a given software requirement is not installed or set up properly, you will get a message with detailed instructions (tailored to your operating system) on how to set up your computer.<!-- CJ I think you should explain your philosophy that the package provides additional tutorial instructions depending on the user's situation before diving into the actual tutorial. I foresee that people are going to be puzzled when the tutorial steps present them with additional mini tutorials. Explain in the introduction what the problem is (users need to install andfamiliarize themselves with many different software solutions) and how you solved it (you wrote a package that tells users what dependencies they need to install and how -->
...
# Planning Phase <!-- CJ This reminds me of the WORCS phases, which is fine, but it's the only header that does remind me of these phases. You've not introduced a breakdown of the research process into phases anywhere. Moreover, the text below does not really relate to "planning"; it goes back to reasons for reproducibility, and then an explanation of technical tools (Rmarkdown and Git), neither of which relate to planning! -->
...
Therefore, we suggest starting early, ideally from the moment you decide to pursue a project to incorporate tools to help you achieve reproducibility gradually.<!--CJ The three preceding sentences belong in the introduction; this tutorial part should be more applied, less conceptual-->
...
We recomend the [`here`](https://github.com/r-lib/here)[^here] package [@R-here] to refer to files, because it works consistently across different operating systems, and between R scripts and [R Markdown].
...
<!--Keep notes.-->
 A great tool to capture ideas or meeting notes and eventually complete entire journal articles is Markdown.
 ...
 This way, Markup<!-- CJ Up or down?--> does not need any specialized software for editing, it is highly fool-proof (unlike for example LaTeX) and can be ported to various document formats, such as HTML Websites, a Microsoft Word document, a typeset PDF file (for example, via LaTeX journal templates), or a Powerpoint presentation.
...
We will later see how this simplicity enables us to create many different document formats (i.e. Word, PDF, HTML) and incorporate code in different languages (i.e. R, Python, Julia).

## Git

Another advantage of Markdown is that because of its likeness to plain text<!-- CJ This will not make sense to someone who does not yet know how git works and why plain text is preferred, i.e., your target audience-->,
you can easily version-control it with Git and leverage Gits features for tracking, displaying, and merging sequential and concurrent changes.
You may be familiar with Microsoft Words' "Track Changes"-Feature.
Version control <!-- CJ You're now introducing version control after using it in a sentence (preceding). That's too late. Moreover, as the paragraph is labeled "Git", I feel that you should explain git, not version control. Also... shouldn't tools be introduced in the introduction, instead of the tutorial? The tutorial should show users how to use tools that have been explained to them in the introduction.--> 
  is similar in its basic idea but more potent in its features.
...
For example, this paper has been going through `r system2("git", c("rev-list",  "--count",  "HEAD"), stdout = TRUE)` `r link("https://github.com/aaronpeikert/repro-tutorial/commits/main", "iterations")`. <!-- CJ Weird flex, readers will only see the number and won't know that it was dynamically generated from a git call. They also won't understand how that number conveys anything about the benefits of git.-->
...
And you could commit it in Git.<!-- CJ Do people know what a commit is yet?-->
For convenience, we use the [`gert`-package](https://github.com/r-lib/gert) [@R-gert], which wraps most git commands into R functions.<!-- CJ Actually, I believe gert is an independent git client - but not 100% sure-->
Alternatively, we suggest to either work directly in the terminal (for example, RStudio offers a command shell with various options accessible in the Menu Tools → Terminal), or work using the Git pane in RStudio that offers buttons for basic operations.
<!--I am leaning towards doing everything in R, not sure though-->
<!--For Git we maybe should use RStudio's Git pane?-->
<!-- CJ No preference yet. I think command line R is convenient, but the R git pane gives a nice overview of new and changed files etc. That would be very instructive to new users. Maybe you can demo how the command line relates to the git pane? I.e., the git pane will be updated when using gert::git_add()-->
To commit your first changes to the project, start with staging all files <!-- CJ do they know what staging is yet? Do they need to know?--> to be included in the commit using `git_add()`.
...
Now assume that after discussing with your colleagues, you decide on a different strategy for the final analysis <!-- CJ I missed what the original plan was-->: using latent variable models with tests for measurement invariance <!--CK consider dropping, it's not essential and confusing to readers who don't know SEM: because power is greater and you want to exclude bias in measurement as possible confounder--> [@flake2020].
...
And GitHub can show you what has changed, like in Figure \@ref(fig:idea-change). <!-- CJ Is the project linked to github yet? Have the changes been pushed? Do readers know what github is yet? -->
This detection of what has changed works on a line-by-line basis. <!-- CJ this should have been explained when you first introduced git -->
We recommend therefore to keep each sentence in a Markdown on its own line to generate clean difference views.
GitHub is, in its essence, a web interface to Git, and you may learn more about how to use it under the section on [GitHub]. <!-- CJ This should have been introduced earlier-->
...
<!-- CJ I think you should avoid teaching people git. You're not teaching them R or how to use RStudio either. Instead, teach readers only the absolute minimum they need to use it. If possible, consider importing the git_update() function into repro, because that will save a lot of explaining in the paper. Do provide a link to a good resource for people who want to learn more; I believe github has tutorial pages.-->
<!-- CJ Also, I feel like most of what is said about git below here should have been in the introduction. Some other snippets (e.g., explaining commit, push and pull) should have been presented before you tell users to make commits. It comes too late here-->
...
However Git is designed for collaboration, we therefore recommend finding a partner to attend an online lecture or workshop to learn the ins and outs of Git.<!-- CJ I think this is a good sentiment; however, I would (also) provide a link to a good tutorial, and emphasize that all they need to know to get started will be covered in this tutorial, otherwise people get frustrated that they have to go read another tutorial halfway through your tutorial.-->
...
Monte Carlo simulations <!-- CJ Explain what this is! Or consider using plain language instead.--> are a crucial tool to investigate properties of a statistical model and they are routinely employed to develop or evaluate statistical models.
...
While rather uncommon for an empirical research project, we argue that simulating data offers various benefits in the planning phase of a study.<!-- CJ I feel that this is a better opening paragraph, because it introduces the concept of simulating data! However, consider that some readers may have no idea of what that means, and might require even more detail.-->
...
A Monte Carlo simulation affords you an ideal planning opportunity.<!-- CJ Put the other monte carlo stuff here, and explain what it is-->
We realize that this comes at an expensive but believe the reduction in the risk that the data is not sufficient to answer the research questions offsets the required effort.<!-- CJ What are you trying to say in this sentence? -->
This advantage is especially pronounced when you plan to preregister your study.
In fact, a Monte Carlo simulation can help plan the analysis in detail, form the basis of power calculation and sample size planning, and can be preregistered integrally (see [Preregistration as Code].<!-- CJ Should this preregistration as code have been introduced earlier? There, you could discuss the notion of simulating data-->
...
The estimates for the construction sample, which was recruited from Amazon’s Mechanical Turk (MTurk) and consists of 489 adults from Canada and the United States, can be seen in Table \@ref(tab:loadings))
...
These assumption can be simply<!--CJ I wouldn't use this word here. People might find this all quite complicated, and could be disparaged if they read that this should be simple--> translated into [`lavaan` [@R-lavaan]](https://lavaan.ugent.be)[^lavaan] model syntax:
...
The following code simulates data from the population model `model_truth` with 1000 cases in each group. <!-- CJ I'm left wondering whether the example shouldn't simply be a t-test with observed variables. Everybody knows linear models. Lavaan is quite a specialized piece of software, and you might lose a lot of readers when the examples relate to SEM and require understanding of factor loadings, model implied covariance matrices, etc -->
...
We propose the following procedure for analyzing the data and dealing with possible violations of strong measurement invariance. <!-- CJ Again, mweasurement invariance is probably too technical for an introductory example in a paper about workflows, not SEM-->
After fitting the model with strong measurement invariance [@flake2020], we automatically free equality constraints, first on loadings, and then on intercepts based on the Lagrange Multiplier test. <!-- CJ This is really getting too complex for an example -->
...
generate_data(1000) %>% 
  ...
  extract_results()
```
<!-- CJ I would just use an example as simple as something like this:
x <- rnorm(100)
y <- .2 + 2.3 * x + rnorm(100)
That's already going to be a big step for many, but hopefully manageable. After grokking that, they can get back to the true point of the paper: Workflows.
-->
...
Such Monte-Carlo simulation can help to decide between several trade-offs.
Historically, one important trade-off is between sample size and power (the probability to detect an effect of given size if it exists).
However, the question can be put more broadly and it is possible to consider various other trade-offs of relevant study design parameters other than sample size, such as number of measurement occasions [e.g., @brandmaier2015lifespan], reliability of measures [e.g., @brandmaier2018precision], or parameters related to the structure of planned-missing data design [@brandmaier2020optimal].
To show that there can also be alternative approaches for sample size planning other than those that are restricted to null hypothesis testing, we choose to visualize the precision of estimation as the density distribution of the parameter of interest (latent mean difference by gender of Machiavellism) instead of statistical power.
Such process still allow us to formulate expectations for the expected precision of the chosen analysis. For example, with a sample size of 1,000 per group, 80% of the simulated standardized effect sizes fall into a range of +/- .1 standardized difference points. This claim is useful and informative irrespective of whether we embark on null hypothesis testing later on or not.
The R code to reproduce the simulation and the figure can be found in the accompanying [GitHub repository](https://github.com/aaronpeikert/repro-tutorial)[^gh-repo]:.
...
To consistently get the same results while preserving the pseudo-random property of the generated numbers, it is good practice to set a random seed that sets the PRNG into a restorable state, in R code: `set.seed(1235)`.
Note however that the consistency of a given seed is not necessarily maintained across different R versions because the default algorithm for generating pseudo random numbers may change (see @Peikert2019 for a demonstration with different R versions).
It is a little more complicated to achieve uncorrelated random numbers in parallelized code (e.g., when the same code is run with different portions of data in a computer cluster).<!-- CJ This is probably too specialized for a paper about workflows. You have not introduced parallelization, so it doesn't make sense to talk about special provisions for paralellized code. -->
...
While this RNG is available as a low level interface in R (with `RNGkind("L'Ecuyer-CMRG")`), we rely on the excellent parallelization framework around the [`future`-package](futureverse.org)[^futureverse] to handle this for us across all workers:<!-- CJ These are many technical terms that apply only to parallelized code; a concept you have not introduced and one that seems out of scope-->

```{r}
# reproducible but unsafe in parallel code
set.seed(1235)
runif(1)
set.seed(1235)
runif(1)
...
By wrapping code that uses the internal random number generator (here, the command to create a random number from the uniform distribution, `runif(1)`) with the `future` approach, we guarantee that the random numbers will differ across parallel executions of that code, will achieve a high quality of randomness, but still will be deterministic and thus reproducible:

```{r}
# reproducible and safe in parallel code
...
Thus, it serves as a tool that may help to increase the credibility of empirical results.<!-- CJ For this sentence to make sense, you have to briefly mention the problem with non-preregistered studies: Undisclosed exploration, HARKing, and the file drawer effect-->
Without getting into too much detail of the meta-scientific arguments for preregistration <!-- CJ I think you do need to mention some of those arguments.-->, we want to note two advantages [@nosekPreregistrationHardWorthwhile2019].
...
A preregistration is essentially a detailed research plan; the only difference is that a preregistration is made public <!-- CJ Not necessarily! prereg can be deposited in a private or embargoed repository. The crucial point is that it is time-stamped and indelible, so that future auditors may verify correspondence between the prereg and final analysis. --> beforehand.

The second advantage is transparency.
A preregistration offers the interested reader more insight into the research process than a paper alone could.<!-- CJ Yes, but is that so important? -->
...
While such an argument has some merits, we believe that often transparency outweighs these concerns [@nosekPreregistrationHardWorthwhile2019]. <!-- CJ Not clear how transparency outweighs (?) concerns about inflated false positive rate.-->
Researchers may have good reason to deviate from a preregistration; for instance, if real data are found to violate assumptions (e.g., about distributions, or measurement levels) that were made at the time of study planning.
To account for such unforeseen circumstances, there is some room to deviate from a preregistration - provided the deviation is clearly disclosed.
A preregistration, together with version control, allows their readers to inform themselves about these lessons learned. <!-- CJ I would emphasize here that version control functions as a research log / diary, which shows how a project progressed from idea to prereg to preliminary analysis to corrected analyses with some deviations from the original plan. If the project log is straigfhtforward, with some disclosed deviations from prereg - the research is credible. If the project log is convoluted as f***, then it's not credible.-->
...
All of these templates use natural language to describe the analysis plan.
The crucial limitation of any natural-language representation of an analysis plan is that it leaves room for ambiguity when translating the plan into code [@sep-ambiguity].
Preregistration as code (PAC) addresses this limitation, by... <!-- CJ Provide definition first.-->
...
Before researchers gather data, they write a reproducible, dynamically generated manuscript (see [R Markdown]) including the sections Introduction, Methods, and Results---initially based on simulated data.
...
One particular advantage of this approach is that you can precisely define if-else conditions under which you choose a different analytic strategy to address possible complications (e.g. to establish partial measurement invariance, when full measurement invariance is not attainable). <!-- CJ This is a great idea, but I feel like you should first explain the general concept of PAC in more detail before presenting specific benefits, like forking paths.-->
This approach allows for more flexibility <!-- CJ This is confusing, because you just said that traditional templates allowed for too much flexibility--> than a traditional template based preregistration but is still well defined.
...
One way to ensure that your analysis to works with real data is to simulate data. <!-- CJ I'm having flashbacks to the previous paragraph about simulating data. Haven't you said this already? Merge all sections about the same topic-->
...
Above, we have already suggested [Markdown] as a universal and portable text format. <!-- CJ I feel like these sections should be merged. It does not make sense to introduce markdown first, and then several pages down, introduce R Markdown.-->
...
If you have simulated data, you could even analyze them to include dummy results.
...
An essential aspect of a preregistration is that you have to make it public. <!-- CJ This requires a reference. I believe I found one that defined prereg as an "indelible, time-stamped record". That's useful-->
Of course, if you commit the preregistration to Git and upload it into a public repository, it is also public. <!-- CJ You should credit my paper for this, as I'm the first to make this argument.-->
...
You can learn more about this in [Creating a public release].<!-- CJ That's a great idea! -->
...
### README (optional) <!-- CJ I think this is too much information about the readme file. -->
...
The `README.md` file will look pleasant in the browser on [GitHub](https://github.com/aaronpeikert/repro-tutorial#readme).
...
In our experience <!-- CJ This sentence should not be used in relation to legal matters. Refer to a trusted legal authority, not personal experience. -->, the [Creative Commons - Attribution license (CC-BY)](https://creativecommons.org/licenses/by/4.0/) is often appropriate for sharing texts, R Markdown files, generated figures, and other media (such as stimulus materials if you created them).
...
Copyright does often not apply to data <!-- CJ Reference, also this is a very important point that might require some elaboration.-->, we therefore recommend to share data with a CC0 license to clarify that you do not hold copyright to it.
...
However, if their first contribution occurs after adding a license, they give their consent by contributing.<!-- CJ Is this true by default, or should it be included in your contributor guidelines or someting?-->
...
### Code of Conduct <!-- CJ I have mixed feelings about this. Considering that CoC is a recent development in software communities, and that it has caused substantial debate (and some casualties) in those communities, and considering that it is not currently "a thing" in scientific communities, and considering that it does not directly relate to reproducible workflows - I sincerely doubt whether this has any place in the present tutorialn. I would recommend sticking with solutions that solve issues experienced by scientists, not by programmers.-->
...
## Collaboration <!-- CJ This should be in the introduction - in fact, there was already a remark about it. Merge those sections -->
...
### GitHub <!-- CJ merge this with your earlier github section -->
...
### GitHub Issues <!-- CJ This is another example of something not essential for reproducibility; consider leavcing it out of the paper. Definitely not worth it's own header! -->
...
### GitHub Pull Requests <!-- CJ Same, not relevant -->
...
Before incorporating changes from someone else, they can be reviewed and adapted.
This review step is particularly useful for remote collaboration, because collaborators can be explicitly pointed to changes and asked to approve them.
Therefore, it is sometimes useful to create a pull request even though you have permission to push, because it invites your collaborators to comment on, change, or approve proposed changes.
...
This process can be executed via Github's web interface and can work well with collaborators who do not actually want to bother with installing and running the entire code base of a project.

# The Data Analysis Phase

So far, we have covered the generation of dynamic documents with [Markdown] and [R Markdown], and tracking changes with [Git] and [GitHub]. These are two of the four components [see @Peikert2019] to achieve long-term reproducibility. To complete computational reroducibility, researchers need to properly handle dependency relations between their files (internal dependencies) and all dependencies on external software, most importantly dependend R packages (external dependencies). In the following, we will show how the `repro` package supports researchers in mapping all dependencies completely and unambiguously.

## Internal Dependencies

<!--I am not sure this is the right place, should we introduce this earlier? --> <!-- CJ Yes-->
...
This modularisation <!-- CJ Are you using UK or US spelling? Modularization? --> facilitates reuse because the loading script may be used by other projects building on the same data, while the analysis script may be employed on different data and so forth.
...
One could build an R package to separate analysis <!-- CJ This sentence is going to lose all readers who are not R package developers, i.e., the entire target readership--> and data or use code externalization <!-- CJ Now you're even losing me ;) --> in the RMarkdown to facilitating reuse or create a "main" script that combines all steps.
We found Make to be particularly useful for two reasons. <!-- CJ Unless I'm mistaken, Make has not yet been introduced? -->
First, it is language agnostic, therefore fusing steps that are carried out in different programming languages. <!-- CJ This is confusing, because you have not yet explained what make is / does -->
Second, it does implement a simple yet powerful caching mechanism <!-- CJ Eh?? What is this --> of results which is welcome in computing-intensive analysis often found in neuroimaging or machine learning.

Make is build around the concept of computational recipes.<!-- CJ This is better. Start with this (and add one sentence first with a definition of Make-->
...
The procedures are formulated as shell commands. <!-- CJ This might shock you, but many readers won't know what shell commands are. -->
...
If a target file already exists, Make only recreates it if necessary.
If a dependency is newer than its target, make recreates the target to reflect the updated dependency.
In our case, when we (re)build a manuscript and some dependent data file has changed, the data analysis is automatically rerun.
This avoids mistakes in the reporting that are due to reports and analyses being out-of-sync.

<!--
AP: I suggest to remove this simply because it distracts from our primary tool and we need to shorten
One alternative that is even smarter (in terms of automatically inferring dependencies) than Make is the R package [`targets` [@targets2021]](https://books.ropensci.org/targets/)[^targets].
`targets` can infer more about your project than Make, because it analyzes the R code you write to build the dependency tree.
...
Writing Makefiles manually is cumbersome for larger projects. 
Therefore, the `repro` package provides means to automatically write Makefiles for a given R project.
To this end, `repro` extends the YAML metadata header of R Markdown files. `repro` introduces a new first-level tag `repro:` with second-level tags (`scripts:`, `packages:`, and `data:`) that take lists of dependent files. For example, if an R Markdown document depends on a further file with R code, the header would look like this:
...
To automatically generate the Makefile, `repro` provides a command that goes through all R Markdown files in the project, parses their YAML metadata, and infers a `Makefile` from them:

```r
automate_make()
```

The generated `Makefile` automatically contains separate targets for each R Markdown documents.
The names of the Makefile targets are identical to the Markdown file names.
For example, to generate the Markdown file `README.md` from the R Markdown `README.Rmd`, you can run:
...
It is common practice to name the very first target in a Makefile "all" and have it include all important files of the project.
If you do not specify a target when calling Make, Make defaults to this first target.
`repro` reminds you of that common practice and suggests which file you might want to add to the target `all`.
Thus, if researchers who try to reproduce a given project and are unaware of its potentially various contents, will always build all reproducible artifacts of a repository when calling Make without further arguments:
...
Running this command fully reproduces the entire project -- but only if you have all required software installed. Next, we will describe how `repro` helpts to manage all software dependencies automatically.
...
Modern data analysis is only feasible because researchers rely on much external software. <!-- CJ This is kind of a sweeping statement... what does it mean?-->
...
External dependencies are unavoidable but, at the same time, a major threat to long-term reproducibility.
The critical problem is the fragility of a software setup.
Many components have to fit perfectly together, and each change, e.g. a software update or bug fixe, risks that balance.
...
While you can do so manually, we suggest to again use Make ( see [Internal Dependencies]).
The `Makefile` built by `repro` does contain a recipe for creating the `Dockerfile`.

```bash
make docker
```

Importantly, the `Makefile` created by `repro` includes a mechanism to execute commands in the Docker container instead of locally.
This mechanism is triggered by the argument `DOCKER=TRUE` appended to any make call. 
The following call builds all targets inside the container instead of in the local computing environment:
...
Last, to create or update the container and all Make targets, `repro` provides a single function `automate()` that calls `automate_docker()` and `automate_make()`.
 
You can also manually edit the `Dockerfile` by changing the file `.repro/Dockerfile_manual` to install software other than R packages.
Remember to run `repro::automate()` every time you made edits to `.repro/Dockerfile_manual`.
...
Setting up and running Docker is not always painless, because it requires a complicated mental model of the underlying virtual computer and, specifically, knowledge about Linux environments.
...
Thereby, the repro package makes massive parallel computing in heterogeneous computing environments less error-prone while ensuring the full documentation of all computational steps taken — even if some or all of them were run in parallel on distributed platforms.
...
In a public repository, you may want to mark important stages of your project, like preregistration, preprint, submission and acceptance at a scientific journal.
We suggest to archive these milestones individually.
A reproducible project is only then properly archived when it is stored at least at two independent storage locations and reproducible from all locations. <!--AP citation? -->
To really make sure that a given project is reproducible, one should aim for an explicit test of reproduction before the project is published. 
We recommend to check out and reproduce the project in a new directory in a "fresh" environment (in particular, a new R session should be used for this). 
For maximal validitiy of this test, we suggest to ask a coworker or collaborator to reproduce the project independently from you.
...
When you disseminate multiple versions of a document (e.g., at different stages of a review process), it seems good practice to specify the exact version of code and data used to create a given document (e.g. the final type-set version of a journal article).
To create a clear link between the repository version and a given end result you can automatically include the unique hash identifying the project state in the git version history.
...
```
-->
To this end, we use the `gert` package:
...
Typical open research project will have their contents available from multiple sites, e.g. from GitHub, preregistration platforms, preprint servers, or publisher websites. 
Under these circumstances it is important to provide unique links 
...
The DOI can also contain information about the license of its target.
<!-- public releases like preregistration, preprint -->
zenodo.org [@zenodo] is a publicly funded service provider that archives digital artifacts for research and provides DOIs for these archives.
...
> Log into zenodo.org → Account → [GitHub](https://zenodo.org/account/settings/github/)[^zenodo-github]

[^zenodo-github]: https://zenodo.org/account/settings/github/

After you have linked a GitHub repository, you trigger the archival by creating a GitHub release, as described in the next section.
...
A central point of criticism is that the proposed workflow introduces a bulk of software solutions and services that may not be available in the long run.
First and foremost, it relies on R, RStudio, Pandoc, Git, Make, Docker, Homebrew (OS X only) and Chocolaty (Windows only) as software and on GitHub, DockerHub, MRAN and Ubuntu Package Archive as services.
These many breakpoints can be reduced to a single service and a single piece of software.
...
We already recommend to store all project files redundantly (your computer, a remote git server such as GitHub, and a publicly funded, open-science-compatible service provider such as zenodo.org).
Building the Docker image from the Dockerfile still requires DockerHub, MRAN, and Ubuntu Package Archive to be online.
We therefore suggest that you include an already built image into the releases.
The Makefile generated by `repro` does include a target to do so.
...
This image allows anyone with a computer equipped with Docker to reproduce the analysis (if the repository contains all code and data and has no other external dependencies).
Last, one can convert the Docker image to a full bootable system image.
Such a system image can subsequently be booted on any personal computer without relying on any software or service and therefore grantees reproducibility for decades to come.
Currently, `repro` does not yet facilitate this conversion, but we hope to include this feature soon.
