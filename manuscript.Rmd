...
Only a reproducible object can be tested and understood by anyone.
...
This is because making a research project with all code, data, and materials reproducible comes with a variety of challenges (e.g., missing software dependencies or ambiguous documentation of the exact computational steps taken), and there is a dire need for reproducibility standards.
With the technological advancement, it is now possible to make all digital products related to a research project available so that a research project can be automatically reproduced with minimal effort.<!--AB I don't understand: why is availabbility based on technological advancement? aren't there still be legal restrictions?-->
...
With regard to the first goal, we discuss how to address <!--AB please avoid military wordings--> four common threats to reproducibility with tools originating from software engineering [see @Peikert2019 for details].
...
Ideally, our target reader already has created a dynamic document at some point in time (e.g. with RMarkdown or Jupyter) and knows the basic ideas behind version control (e.g. with Git). 
...
Another property of a computational recipe is that the same inputs <!--AB of what? I deleted 'of'--> should always result in the same outputs.
...
Whenever random numbers are used in a computation, this computation is only reproducible when the samerandom numbers are used.
To ensure identical random numbers, users may fix the state of the random number generate with a so called seed (e.g. in R with `set.seed()`) but they also need to guarantee that the pseudo random number generator is unchanged (see @Peikert2019). <!--AB please check if citation is rendered correctly-->
...
We recommend [`renv`](https://rstudio.github.io/renv/articles/renv.html) [@R-renv] in cases where no administrator rights can be obtained but can not detail its use in this document. <!-- add a half sentence about what R-env is-->
...
We can only recommend PAC when the dangers to reproducibility we described above are eliminated and a state of automated reproducibility is reached. <!--AB I do not understand this last sentence-->
...
Nevertheless, they may obfuscate the real data (before actually looking at it) by shuffling all variables per each colum (independently from each other) in cross-sectional data <!--AB would not work well in longitudinal data--> or otherwise creating synthetic data that allows them to test the function `planned_analysis()` without actually revealing the observed research outcomes.
...
Guidelines for clinical trials also recommend analysis of blinded data <!--AB I added data here?!--> to test the practicality of the preregistration [@ICH1998].
Since the data is obfuscated, virtually no results<!--AB do you mean observed data? what are results here?--> are revealed and the researchers do not gain researcher degrees of freedom.
...
To be clear, any exploration of the data before preregistration renders the preregistration invalid and amounts to scientific (albeit sometime unintentional) misconduct.
...
However, we must warn our readers, while piloting is more traditional than our approach of obfuscating the data before preregistration, it may not be innocuous to use the resulting data and care must be taken to avoid errors of sequential testing [@whiteheadBiasMaximumLikelihood1986]. <!-- this may not be clear to everyone... do you mean that you cannot reuse the pilot data?-->
