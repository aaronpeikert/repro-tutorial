...
acknowledgement: |
  We would like to thank Maximilian Stefan Ernst for his contributions to the code for the simulation study. We are grateful to Julia Delius for her helpful assistance in language and style editing.
authorcontributions: |
...
First, we want to introduce researchers to a notion of automated reproducibility that requires no manual steps, apart from the setup of the software. 
Second, we present the opportunities such automated reproducibility affords to improve not only how research outputs are created and recreated but also how research itself is conducted.
...
If some parts of the workflow prove to be unfeasible, we encourage the reader to find their own solutions and to concentrate on the feasible parts.
...
@bakkerRecommendationsPreregistrationsInternal2020 found that a formal power analysis does not increase the planned sample size on average but can help to invest resources more efficiently.
...
This means that we did need to change our preregistration after accessing the data to include i.e. the recoding of gender (lines 17-22) and the items (lines 23-26).
We invite the reader to evaluate the changes we made to the preregistered code on [GitHub](https://github.com/aaronpeikert/repro-tutorial/compare/v0.0.1.1-prereg..main#diff-e21a8fa2e44b297dfefef329a6ef56d283488d467c4b4ffe2a014111e52a170b).
...
such as preregistration as code (PAC).
...
The likelihood of such deviations, remains unavoidable, but is decreased by using simulated data to verify the suitability of the proposed analysis plan.
This analysis plan may contain data-dependent decisions to increase the flexibility of research plans.
