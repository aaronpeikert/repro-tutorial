...
Ideally, our reader already has created a dynamic document (e.g. with RMarkdown or Jupyter) and has tried their hand once at version control (e.g. with Git). 
The R package `repro` supports researchers in setting up the required software and in adopting this workflow.
We present the automated reproducibility as best practice, a goal that is not always archived because of limited resources, technical restrictions or practical considerations but is worth striving for.
...
Each entry in the change log should contain the commit id to the respective changes.
The reader is also in a position to inspects the changes one by one and can therefore make an informed judgment about their implications for the inferences drawn.
...
However, we share the common view that a deviation from a preregistration is not a problem; it is a problem if these changes are not made transparent.
...
Such traditional exploratory results should merely be explicitly declared in the manuscript to allow appropriate judgment of the results. <!-- citation? The Nosek paper?-->
The second kind of exploration is through procedurally well-defined exploration with exploratory statistical models, which are standard in machine learning. <!--citation? reproducibility in machine learning research? maybe cite the Brandmaier (in press) chapter, where I suggest to use PAC for these kinds of models? -->
...
While capture the tasks here in the form of explicit R-functions but researcher may use any form (e.g. scripts) as long as it serves to simulate, analyze, and report.
