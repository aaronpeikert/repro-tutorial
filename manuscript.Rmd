...
With simulated data researchers can check if the analysis recover the known parameters of the simulation.
...
A variety of packages automatically generate well formatted Markdown outputs of statistical reports or even entire tables of estimates or figures directly from R objects.
...
Ideally, these three functions can be composed to create a "fake" results section, e.g. when composed to `report_analysis(planned_analysis(simulate_data()))` or `simulate_data() %>% planned_analysis() %>% report_analysis()` they output a full results section.
...
We have made the argument that PAC has great advantages over classical preregistration and have outlined how to implement PAC, but can PAC actually work?
To answer this question and to help researchers implementing PAC themselves, we have conducted a PAC that can serve as an example.
...
Again, we propose a preregistration format that closely resembles a classical journal article but that utilizes simulated data and dynamic document generation to serve as preregistration and final manuscript.
The full example is available in the (https://github.com/aaronpeikert/repro-tutorial/blob/main/preregistration.Rmd).
...
We than translated this methods section into the following `planned_analysis()` function:

```{r planned_analysis}
```

This function illustrates two advantage of PAC.
...
Second, it highlights how difficult it is to precisely describe a statistical analysis verbally.
...
Monte Carlo simulation are of course not only applicable for this analysis method and and also allow researchers to investigate other relevant properties of their analysis method beyond power [@brandmaier2018precision; @harrisonIntroductionMonteCarlo2010; @raychaudhuriIntroductionMonteCarlo2008].
...
For the results section we implemented a mechanism that only uses simulated data when the actual data is not yet available.
...
Please note that the statistical procedure in the example of running different test procedures based on a fixed skewness threshold was created for didactic purposes and it is not necessarily optimal for addressing your research question.
