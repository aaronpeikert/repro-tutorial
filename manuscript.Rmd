...
For a priori planned analyses, preregistration should reduce "researcher degrees of freedom" therefore, the number of decisions researcher can make that could unduly influence the results.
...
The structure these templates provide can more effectively reduce researchers degrees of freedom than an unstructured approach [@bakkerEnsuringQualitySpecificity2020].
Nevertheless, these templates can not remove researchers' degrees of freedom because it is impossible to verbally describe every detail of an analysis for all but the most straightforward analysis.
...
We term this writing and publishing code before data collection *preregistration as code (PAC)*.
PAC has the potential to all but eliminates researchers degrees of freedom that standard preregistration despite the best efforts afford.
...
When leaving meta-scientific benefits aside, a preregistration is a tool for planning a study.
...
Writing incomplete code is technically impossible, or at least researchers would notice if they, e.g. forgot to aggregate the data, and so forth.
...
After they gathered the actual data, they replace the simulated data, render the dynamic manuscript (therefore run `planned_analysis()`) and write the discussion.

## An example of Preregistration as Code

We have made the argument that PAC has significant advantages over classical preregistration and have outlined how to implement PAC, but can PAC work in practice?
To answer this question and help researchers implementing PAC themselves, we have conducted a PAC that can serve as an example.
...
Again, we propose a preregistration format that closely resembles a classical journal article, but that utilizes simulated data and dynamic document generation to serve simultaneously as preregistration and final manuscript.
...
First, a PAC can easily include data-dependent decisions creating different branches of analysis under different conditions.
...
The interested reader can find the example rendered with actual data in the [online supplementary material](this link does not work) and can view the changes we made since preregistering it on this [GitHub page](this link does not work).
...
Please note that the statistical procedure in the PAC example of running different test procedures based on a fixed skewness threshold was created for didactic purposes and it is not necessarily optimal for addressing your research question.
