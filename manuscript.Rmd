...
Because there is a single "source of truth", the dynamic document, including code, under version control, changes can be inspected by readers and authors alike.
To make the comparison more accessible, we highly recommend that the authors review all changes to summarize and justify them for their reader, e.g. in the discussion [^summarize-changes].
...
A reader may falsely conclude that a result is explorative because the publication is more specific than the preregistration, therefore committing an alpha error (believing there is a deviation when, in fact, there is none).
...
We must differentiate here between two kinds of exploration, and neither of them is inhibited.
...
The second kind of exploration is through procedurally well-defined exploration with exploratory statistical models, which are standard in machine learning.
These models, which are difficult to preregister traditionally, can be preregistered with PAC like any other model.
We recommend PAC for exploratory statistical models without any irony.
The merit of preregistration in such cases is to communicate precisely how much exploration was done, information that is crucial for assessing the uncertainty of such now "preregisterable" but still exploratory results.
