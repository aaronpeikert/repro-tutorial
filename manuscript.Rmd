...
<!--Keep notes.-->
 ...
 R Markdown is the smart combination of Markdown with dedicated code chunks that are evaluated to formatted text, statistical results, or images when the document is rendered to its final format
 We suggest to use Markdown for everything, starting from simple sketches of your ideas to your scientific manuscripts and presentations---and even your CV.
...
Now assume that after discussing with your colleagues, you settle onto a final analysis strategy using latent variable models with tests for measurement invariance because power is greater and you want to exclude bias in measurement as possible confounder <!-- Aaron, reference needed-->.
...
Beyond checking the technical side of the model, simulation is also beneficial on a conceptual level [@axelrodAdvancingArtSimulation1997].
...
```{r}
model_truth <-
"MACH =~ 0.27*x1 + 0.22*x2 + 0.28*x3 + 0.36*x4 + 0.41*x5 + 0.5*x6 + 0.43*x7 + 0.32*x8 + 0.36*x9
MACH ~ c(0, 0.2)*1
MACH ~~ c(1, 1)*MACH"
```
...
More complicated simulations (for example, including missing data simulation) are possible with other packages, e.g., simsem [@simsem], or simstudy [@goldfeldSimstudyIlluminatingResearch2020].
The following code simulates data from the population model `model_truth` with 1.000 cases in each group.
...
After fitting the model with strong measurement invariance, we automatically free equality constraints, first on loadings, and then on intercepts based on the Lagrange Multiplier test.
...
Note, that this certainly results in different numbers each time we run it (see ["Randome Number Generation"](#prng)).
...
To complete a simulation study we need a function that repeats the analysis for different conditions (here only for different n):
...
Currently, there are various templates available that guide researchers in developing data analysis plans for preregistration. 
...
If code was version-controlled and tagged (see Section XXX) before data collection, all deviations from the preregistration or any post-hoc additions that were performed after data collection are transparent and are visible and comparable in the version history.
...
The `repro` package is particularly well-suited for computationally intensive data analyses and simulations that  are to be run on distributed computing environments.
By using containers, `repro` ensures identical execution across platforms and thus guarantees that a data analysis or simulation that was designed and tested on a local computer will be executed in the identical manner on a remote machine or various remote machines across an entire computing cluster.
...
Singularity is an alternative solution.
It was designed to run without administratot rights, while maintaining full compatibility with Docker images.
...
Because you probably want to reproduce your own project each time something changes, you will often want to reproduce evertyhting.
The `repro`-package contains a function designed for that purpose:

```{r}
repro::rerun()
```

This function automatically figures out how to reproduce a project and also recognizes other employed reproducibility tools (like `renv` and `singularity`), but can of course only guess the appropriate way to reproduce a project.

If you have written a [Preregistration as Code] you can swap the data file and reproduce the project.
In fact, before the`r link("https://github.com/aaronpeikert/repro-tutorial/releases/tag/ v0.0.1-preregistration", "preregistration")` were the results below on simulated data, only after we preregistered the whole manuscript, they were based on actual data from https://openpsychometrics.org/_rawdata/.
...
A prerequisite to reproducibility is the availability of code and data.
We use containers to guarantee that all code components are available and work as expected.
Ideally, all data is also stored in the repository [for a more comprehensive argument for publishing data see @vanlissa2020worcs], but this is not always feasible.
