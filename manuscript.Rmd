...
We therefore suggest to start early, ideally from the moment you decide to pursue a project to gradually incorporate tools to help you archive reproducibility.
The first step is to organize all files in one folder.
This way you never loose anything and you can easily share the files.
<!-- Hands-on: Introducing RProjects-->
This concept of a project folder is facilitated by RStudio's RProjects.
...
You may be familiar with Microsoft Words' "Track Changes"-Feature and version control is similar in its basic idea, but more powerful in its features.
Instead of tracking only one file, you can keep track of the whole project directory and instead of loosing all changes when you agree to them, the whole history is preserved transparently.
...
Recording each change you made, helps you to iterate more quickly because you know that you can go back to previous versions easily.
...
First you could create a new file named `idea.md` that looks like this:

```markdown
# Hypothesis

Machivallianism is higher in male persons
...
We could use openpsychometrics.org data?
```
