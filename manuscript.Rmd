...
First, limitations by design that are unlikely to be changed in the near future.
This workflow does inherit these both from the software it does rely on and the fundamental design principles this software shares with the workflow and `repro`.
Second, limitations that we hope development effort of the open source community and ,with regard to `repro`, us will ease.

As a principle, this workflow is incompatible with steps that cannot be automated.
This means many researcher will have to search for alternative software that allows for automation and it is at odds with some ingrained habits of researchers to mix and match manual and automated steps in data analysis.
This overarching principle manifests itself as several technical but nevertheless important limitations.
...
`Make` can orchestrate any software that is programmable but fails to automate software that will only obey a user clicking at the right buttons.
`Docker` can ship any software that runs on linux and can be automatically installed but that precludes many commercial or closed source software.
...
On the one hand, we firmly believe that automated reproducibility makes research more productive and collaboration easier but on the other hand we expect that researcher invest considerable time in learning these tools and than must "force" their collaborators to do the same.
Two arguments reconcile this apparent paradox.
...
Researchers can pick up one skill at a time and than help their fellow collaborators to do the same.
Second, the upfront investment is one time (and efforts such as `repro` are underway to reduce it) while the complexity and demands of research projects are steadily increasing.
