...
With the technological advancement, it is now possible to make all digital products related to a research project available in a way that enables automatic reproduction of the research project with minimal effort.
...
To ensure identical random numbers, users may fix the state of the random number generate with a so called seed (e.g. in R with `set.seed()`) but they also need to guarantee that the pseudo random number generator is unchanged [see @Peikert2019].
...
This lockfile is than used to reinstall the same packages on other computers or later in time.
For a more thorogh discussion see @vanlissa2020worcs.
