...
The end goal of this workflow is a project that can be completely, automatically, and unambiguously reproduced.
...
and automatically reproducible with a single command.
...
We have developed a workflow that leverages established tools and practices from software engineering to achieve long-term and cross-platform computational reproducibility of scientific data analyses resting on four pillars that address the aforementioned causes of non-reproducibility [@Peikert2019]:
...
The following snipped, for example, installs Python but works the same for any other software users might want to install.
...
These function again should accept the output of `planned_analysis()` as input.

The goal of this function is to describe the results to readers automatically.
This way, the preregistration specifies not only the computation  also how the computation's results are reported.
...
This function receives a dataset as input and performs all statistical processing.
...
This simulated dataset fits perfectly into our planned analysis with a sample size of 400 and 10 items:
