...
Scientists increasingly strive to make research data, materials, and analysis code openly available <!-- AB "i.e." does not work in this context. I just removed this because it's irrelevant whether this happens within the movement or not?!-->
...
This is because making a research project with all code, data, and materials reproducible comes with a variety of challenges (e.g., missing software or ambiguous documentation of all steps taken <!-- AB the previous list was difficult to understand without more context; these two points are hopefully better to understand--> ), and there is a dire need for reproducibility standards.
...
<!-- AB Absätze können nicht nur einen Satz haben! Außerdem fehlte das "Second"-->

<!--AB "This" geht nicht am Absatzanfang--> The proposed workflow builds on several technical solutions originating from software engineering. 
...
<!--The first part... dieser Satz wirkte verloren. Habe ich gestrichen-->
...
<!--why boldface? looks odd and it is easy to confuse with section headings-->
1. Version control
...
<!--AB according to our own license in the original paper, we need to state the license under which the image was released. Please complement this information.-->
...
For example, a verbal description of a statistical model may be unidentified, e.g. if it includes reciprocal paths between variables, or more parameters than observed data. <!-- AB note though that we still cannot exclude the possibility of empirical underidentification with this approach. -->
...
The workflow described in this paper facilitates a rigorous solution to this problem: <!--AB is it really innovative? I guess since people have preregistered code before, it's not entirely new... even though the reproducibility aspect is innovative-->
...
Moreover, it removes the necessity to write two separate documents: A preregistration _and_ a manuscript.
...
One particular advantage is the foundation in existing literature where the introduction and theoretical background provide the basis for the design of the study.
...
@bakkerRecommendationsPreregistrationsInternal2020 found that a formal power analysis does not necessarily increase the planned sample size. <!--AB I am not sure what you want to say here. Is this saying that researchers usually already have a good hunch and they should not be afraid? I think this needs more explanation -- or remove the sentence-->
...
For basic research projects, we suggest writing three functions for each planned hypothesis, one for the planned analysis, one for simulating the data and one for reporting the results. <!-- AB not sure what this means. Why do I have to write 3 functions for each planned hypothesis? Isn't it one for each hypothesis? Please check -->
...
We argued that PAC has several advantages over classic <!--AB please check different meanings of classic VS classical--> preregistration and have outlined how to implement it.
...
As usual, researchers state why they are interested in their research question in the section "Introduction" and provide the necessary background information and literature to understand the context and purpose of the research question. <!--AB do we explain anywhere that the [@XXX] are bibtex references? This may be not clear to everyone-->
...
<!-- AB I am not happy with the next sentence. Isn't it the other way round? I thought we basically write the analysis first and then translate the analysis into a method section?-->
...
<!--AB this code will not be understood by most of the readers? Can we remove this? Instead keep the results text below but explain what the variables choosen_power and minn mean and that they were either set in the simulation or simulation results #necessary-->
```{r, echo=FALSE}
res <-
...
The workflow we present here rests on four software solutions to guarantee reproducibility: version control, dynamic document generation, dependency tracking, and software management.
...
<!--AB is it correct to claim the following? If yes, there is probably research on this?: Furthermore, adhering to principles of reusability typically removes errors in the code and thus increases the likelihood that the statistical analysis is correct-->
...
<!--AB this paragraph does not really fit here. Either have a subsection heading or, better, move it back up again-->
