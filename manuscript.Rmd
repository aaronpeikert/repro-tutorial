...
For the hypothetical research question, we could for example plan to conduct a simple Student's t-test with Welch's correction an the average of machiavillism items between the gender groups.
As an example of data dependend decisions we included a rank transformation if the skewness greater than 1, converting the t-test to a supposedly more robust Mann–Whitney–Wilcoxon test.[^caution-mww]
This example is suboptimal with regard to several criteria (untested assumption of measurement invariance, underestimation of effectsize in presence of measurement error, overestimation of effectsize for highly skewed distributions), we merely choose it for didactic purposes because similar procedures a widely used (@horton reports that 26% of all studies employ a t-test and 27% employ a rank based alternative in the New England Journal of Medicine in 2005).
