...
Singularity is an alternative solution. It was designed to run without administratot rights, while maintaining full compatibility with Docker images.
...
Note that both this tutorial as well as our article on the reproducible workflow [@Peikert2019] were created using this workflow and can be reproduced from their source code repositories.
<!-- maybe we add here the real data? #25 -->

## Closed Data (optional)

<!-- Hands-on: check changed hash -->
A prerequisite to reproducibility is the availability of code and data. We use containers to guarantee that all code components are available and work as expected. Ideally, all data is also stored in the repository. In practice, however, there are various reasons, which may prohibit you from making your data available, for example legal or regulatory or privacy concerns.
You may find yourself in a situation where you can make the data only available on request.
...
>>>>>>> 2bfdb64 (turn AP callouts into GitHub issues)
```{r}
