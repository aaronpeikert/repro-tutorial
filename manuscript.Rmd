...
These wrappers are merely a support system; as users learn to use the underlying tools, they can rely less on `repro` and use these ttools directly to solve more complex problems.
...
```{r}
# only an alternative: DO NOT USE if you are unsure what SSH means<!--CJ: Does the default method above use HTTPS? This is deprecated by GitHub-->
...
<!-- CJ Explain what the repro project template does -->
...
Note that the `worcs` R-package contains a convenience function `worcs::git_update("commit message")` which adds, commits, and pushes files to a remote repository,
thus functioning as a kind of 'quick save' <!--CJ reference worcs package-->.
...
However, if they have installed `Make` in addition to `Docker`, they do not even need to know how to use `Docker` and can simply rely on the two `Make` commands "`make docker`" and "`make -B DOCKER=TRUE`".<!-- CJ Is this information in the default readme?-->
...
Third, repro constructs `Make` targets so that if you supply `DOCKER=TRUE` they are executed within the `Docker` image of the project.<!-- CJ This paragraph is VERY hard to follow without knowing how make and docker work (you've explained docker a bit, but make not so much). I think you can remove this, it's too technical and not clear why you're explaining this-->
...
Preregistration seeks to reduce researchers' degrees of freedom [@NosekRevolution2018]<!--CJ this is not exactly true; the explicit goal of preregistration is to distinguish exploratory research from confirmatory research. Reducing researcher degrees of freedom is a side effect, not the goal--> in the execution of a planned study by limiting the number of decisions to be made after data has been collected, that could bias the reported results.
Preregistrations have several shortcomings, however.
One is that written study plans can be interpreted in multiple ways and often do not achieve the level of unambiguity that is required to effectively set the researcher degrees of freedom, to zero.<!--CJ I don't think you've defined researcher degrees of freedom? And why is it important to reduce these to zero? Because that blurs the line between exploratory and confirmatory!-->
...
However, the effectiveness of such templates is limited by the ambiguity of using language to describe analytical procedures.
The use of language to describe analytical procedures also complicates comparing the initial plan with the resulting analyses to decide if and how researchers deviated from the preregistration.
