...
Other software that users might need, like other programming languages, not already installed LaTeX packages, etc., must be added manually.
...
The following section explains how to overcome these limitations while still relying on the automation afforded by `repro`.
...
The build date is used to install packages in the version which was available on CRAN on this specific date and can also be changed.
...
However, make sure never to remove the following two lines:
...
To summarise, `repro` provides researchers with an easy to use solution for dependency tracking (in the form of `Make`) and software management (using `Docker`) without the necessity to learn both tools.
...
However, templates cannot remove researchers degrees of freedom completely because it is impossible to verbally describe every detail of an analysis for all but the most straightforward analysis.
This ambiguity causes a second problem, that is, comparing the preregistration and the resulting publication to decide if and how researchers deviated from the preregistration.
...
For example, @steegenMeasuringCrowdAgain2014 realized after publication that part of their preregistered code results in different test statistics than they reported initially (see Footnote 7).
...
Relatedly, writing a preregistration in the form of a research article has the advantage that researchers already have great expertise in writing a scientific manuscript.
...
PAC, therefore, minimizes researchers degrees of freedom more effectively than standard preregistration [@bakkerEnsuringQualitySpecificity2020].
Fourth, despite its rigour, PAC does accommodate data-dependent decisions if these can be formulated as code.
...
@bakkerRecommendationsPreregistrationsInternal2020 found that a formal power analysis does not increase the planned sample size on average but could help to invest resources more efficiently.
...
Using the simulated dataset as a placeholder, we can now write a scientific report including all analysis code blocks and the presentation of results, which can serve as PAC.
...
In order to do so the researcher must walk a fine but important line.
Of course, researchers must not analyze the data before they write the preregistration.
Nevertheless, they may obfuscate the real data (before actually looking at it) by shuffling all variables per each column (independently from each other) in cross-sectional data or otherwise creating synthetic data that allows them to test the function `planned_analysis()` without actually revealing the observed research outcomes.
...
The researcher still can access the information about means or proportions (e.g. how many participants belonging to group "A" are in the dataset) but are unbiased for all relations between variables (e.g. members of group "A" have a greater mean in variable "Z").
...
With traditional preregistration, researchers may omit details and get away with decisions that are made only after having seen the data, thus, potentially biasing statistical results.
Some researchers may not feel ready to decide every detail of the analysis plan before data collection.
In such case, we recommend making decisions based on the best available knowledge, and if these decisions still prove infeasible, make simple assumptions and revise and explain them later as described in the section [Deviating from the preregistration and exploration] of the final manuscript.
...
First, pregistrations of clinical trials (there called statistical analysis plan, @ICH1998) typically describe analyses in exhaustive detail.
Those preregistrations typically contain a detailed description of how results will be presented, including shells of tables and graphics [@yuanGuideStatisticalAnalysis2019].
...
The warning is coloured red to avoid any confusion between mock and actual results.
...
Naturally we did not access the data when we wrote this code initially.
We therefore did not know is the exact format the data would have.
So we did need to change our preregistration after accessing the data to include i.a. the recoding of gender (lines 17-22) and the items (lines 23-26).
We invite the reader to evaluate the changes we made to the preregistratered code on [GitHub](https://github.com/aaronpeikert/repro-tutorial/compare/v0.0.1.1-prereg..main#diff-e21a8fa2e44b297dfefef329a6ef56d283488d467c4b4ffe2a014111e52a170b).
...
But, on the other hand, we expect researchers to invest considerable time in learning new tools and then must persuade their collaborators to do the same.
...
Open data is much more widespread thanks to online storage facilities; preregistration is possible because there are preregistration platforms and so forth.
...
Depending on the demand by users, we will implement some of these alternative workflows in `repro` and hope for broad adoption of computational reproducibility in the near future.
