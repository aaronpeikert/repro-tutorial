...
For simplicity we do not simulate these likely data problems (but doing so would certainly improve the simulation).
...
It is a little more complicated to archive uncorrelted randome numbers when you have parallelized your code, like we did, but the future package handles this for us:
...
To nudge researcher to still be specific enough, there are many preregistrations templates.
...
This is obviously challenging to do, because it leaves no place to hide behind the additional layer of abstraction.
...
An important aspect of a preregistration is that you have to make it public.
...
Second, while Git records meticulously when you made which change, committing and publishing are two different steps.
...
These downsides can be alleviated by using zenodo.org to give a digital object identifier (DOI) to your preregistration, because zenondo.org saves an unchangeable copy of your repository.
...
When you cook something and you have a neccesary ingriedient already at hand, would you throw it away just to prepare it again?
Unlikely.
...
This command will go through all R Markdown files in your repository and infer a Makefile from them:
...
The key problem is the fragility of a software setup.
...
Instead of delivering instructions on how to recreate some required components of an analysis, Docker delivers all software that was used in an analysis, down to the operating system.
...
Such snapshot of the software stack is called "image".
Of course packaging all needed software in such image, requires considerable amounts of storage.
However, two strategies help to keep the storage requirements manageable.
First, there is a community that maintains images for special purposes.
For example, using R [rocker-citation].
Such images only contain what is absolutely necessary for the specific purpose.
...
It does include R, RStudio, LaTeX as well as many R packages like the tidyverse (amounting to `r installed.packages() %>% as.data.frame() %>% filter(is.na(Priority)) %>% nrow()` packages.)
...
Such files are very small (the Dockerfile for this project is `r round(as.numeric(fs::file_size("Dockerfile"))/1024^1, 2)`Kb big), but they rely on the assumption that all software repositories are available and contain the correct software version.
...
These packages together with the R version that you currently used are then translated into an `Dockerfile`.
