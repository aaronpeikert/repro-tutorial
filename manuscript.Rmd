...
simplesummary: |
  ...
  that allows anyone to access a digital research repository and reproduce all computational steps from raw data to final report, including all relevant statistical analyses, with a single command.
  ...
  PAC circumvents the shortcomings of ambiguous preregistrations that may result in undisclosed researcher degrees of freedom when adjustments to the planned analyses are necessary.
...
A single tutorial cannot comprehensively introduce the reader to the individual tools, but this tutorial is intended to help readers get started with a basic workflow.
...
Simulated data allows authors to write their intended analysis script and preregister it<!-- CJ: Aaron: I don't think the key point is that users preregister the presentation of their results (tables, figures), but rather that they preregister the analysis!!!-->.
...
Moreover, contributions (e.g., from collaborators) can be compared to, and incorporated in the master version of the project. 
...
Collaborating via `Git` is facilitated by uploading the repository to a cloud-based service.<!-- CJ This is not true-->
...
One noteworthy property of computational recipes is that they require a textual representation and the ability of a program to make sense of this textual representation.<!-- CJ I don't understand, and is it necessary to explain this?-->
...
A version-controlled dynamic document with dependency tracking still relies on external software (even if just to be rendered to its final output format).
