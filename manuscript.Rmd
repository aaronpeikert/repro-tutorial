...
<!-- AP is there a single command check() that does all of this?-->
...
If you are unfamiliar with the specifics of SEM, we you may skip the next sections and continue in section XXX <!--AP: fill in reference.-->.
...
To add a CC-BY license to your project in general, simply run (there are similar commands for other licenses):
...
You can add other licenses for specific directories (e.g. copy `LICENSE.md` from `usethis::use_cc0_license()` into the `data` folder or the MIT License into the `R` folder).
...
Here, we provided a step-by-step tutorial with what we think are best practices for creating a fully reproducible repository for data analysis projects that center on the statistical programming language R. The workflow avoids common threats to reproducibility, such as missing files, missing software dependencies, side-effects through changing dependent software packages, changes in the programming language, or even at the level of the operating system itself. 
