...
The `repro` also contains a function that automatically figures out how to reproduce a project with:

```{r}
repro::rerun()
```

This function also recognizes other employed reproducibility tools (like `renv` and `singularity`), but can of course only guess the appropriate way to reproduce a project.
