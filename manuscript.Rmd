...
With the technological advancement, it is now possible to make all digital products related to a research project available so that a research project can be automatically reproduced with minimal effort.

We pursue two aims.
First, we want to introduce researchers to a notion of automated reproducibility that requires, apart from the setup of software, no manual steps.
...
With regard to the first goal, we present a tutorial that is aimed at researchers who intend to ensure the reproducibility of a research project and are willing to make relevant code, data, and materials available, whether publicly or on request.
In this tutorial, we introduce the reader to a workflow that maximizes the potential reproducibility of a research project.
The proposed workflow builds on several technical solutions originating from software engineering to automate reproduction. 
...
We further demonstrate how the R package `repro` supports researchers in setup the required software and in adopting this workflow.
In pursuit of the second aim we present a strictly reproducible and unambiguous form of preregistration [@NosekRevolution2018] that builds upon an implementation of this reproducible workflow, the so-called *preregistration as code* (PAC).
...
A computational recipe describes how inputs are processed to create a specific output in a way that is automatically executable.
The concept of computational recipes is central to our understanding of reproducibility because it enables a unified way to automatically reproduce a project.
...
Note that the Workflow for Open Reproducible Code in Science (WORCS) we presented elsewhere [@vanlissa2020worcs] does not contain this element of dependency tracking explicitly but its strict structure of only containing one definite `R Markdown` still makes dependencies between files unambiguous.
...
Such manually build environment can still be ported to all computers which support `Docker`.
We, however, prefer the second way of building images automatically from a textual description called `Dockerfile`.
This enables users to quickly change the software environment e.g. update to another R or package version without being "stuck" with a unmaintainable because manually build `Docker` image.
...
The following section explains how to overcome these limitation while still relying on the automations afforded by `repro`.
...
If the following snipped is added to `.repro/Dockerfile_manual` the `Docker` image will have, for example, Python installed.
...
Even when the researcher was faithful in each aspect to the preregistration the reader may reach opposite conclusions, because the reader has to compare to different texts which may be worded differently or describe the same thing in varying level of detail.
...
For guidance on how to simulate data, see @paxtonMonteCarloExperiments2001 and @skrondalDesignAnalysisMonte2000.
...
The convenient comparison of preregistration and publication with PAC is particularly noteworthy.
Because there is a single "source of truth", the dynamic document including code, under version control, changes can be inspected by reader and authors alike.
To make the comparison easier, we highly recommend that the authors review all changes to summarize and justify them for their reader e.g. in the discussion [^summarize-changes].
The reader is also in a position to inspects the changes and can therefore make an informed judgment about accepting or rejecting the justifications.

Deviations from the preregistration are sometimes demonized as if encountering unexpected aspects would invalidate a whole carefully crafted study.
We, however, oppose this view.
The problem is not deviating from the preregistration; it is not making these changes transparent.
In fact, we expect that most PACs will require changes after the researcher collects data.
Often deviations provide opportunity to learn from the unexpected.
Transparency is important in this regard because an empirical test of a theory can not test the theory alone but tests it in conjunction with auxiliary assumption [@meehlTheoreticalRisksTabular1978].
...
For them, this change simply does not affect their theory's core but was an auxiliary assumption, and they might be right.
Another researcher, however, who, e.g. hopes to incorporate exactly this result into a theory about intelligence, might disagree about the importance of this assumption and would accordingly view the finding as more explorative because of the change to the preregistrations.
...
Transparency of deviations is also preferable to avoiding them by vagueness in the first place.
A vague preregistration must have reduced diagnosticity for demarcating exploration and conformation.
A reader may falsely conclude that a result is explorative because the publication is more specific than the preregistration, therefore commiting an alpha error (believing there is a deviation when in fact there is none).
At the same time, a researcher may overlook a deviation---the goal of intentional vagueness---therefore committing a beta error.
At its most extreme, the preregistration "We will do something with some data." has lost all ability to differentiate between exploration and conformation, which was what we set out to do with preregistrations.

Another common misunderstanding is that, preregistration, and therefore PAC, preclude exploratory analyses.
We must differentiate here between two kinds of exploration.
First, the traditional meaning of ad hoc statistical procedures and post hoc explanation of the results.
Such traditional exploratory results should merely be explicitly declared in the manuscript to allow appropriate judgment of the results.
The second kind of exploration is that through procedural well defined exploration in the form of exploratory statistical models that are common in machine learning.
Somewhat ironically, PAC enables preregistration of such procedures and offers the merit to communicate precisely how much exploration was done; an information that is crucial for assessing the uncertainty of such "preregisterable" but exploratory results.
...
If the PAC is submitted as a registered report [an article format in which the preregistration is reviewed and the article accepted "in principle", @nosekRegisteredReports2014; @chambersWhatNextRegistered2019] a journal article might be more familiar to the reviewers that ensure a comprehensive study plan.
