...
We further demonstrate how the R package `repro` supports researchers in setting up the required software and in adopting this workflow.
...
Because the `Dockerfile` clearly describes which software is installed how, users can quickly change the software environment e.g. update to another R or package version without being "stuck" with an unmaintainable because manually build `Docker` image.
...
We recommend PAC for those exploratory statistical models without any irony.
...
Nevertheless, they may obfuscate the real data by shuffling all variables or otherwise create synthetic data that allows them to test the function `planned_analysis()` without actually revealing the research outcomes.
