...
simplesummary: |
  People trust science because other experts can vet it.
  For example, suppose another person than the author of an article can recreate the same results on another computer.
  In that case, they can more easily trust that the results are correct.
  If anyone can recreate an article, it is called "reproducible".
  We explain common technical problems (e.g. making sure that: 1. nothing has changed, 2. there are no copy-and-paste errors, 3. all steps are in the correct order, and 4. that the necessary software is installed) of reproducibility and show technical solutions.
  However, because these solutions are complicated to use, we have developed some helpers to make them more accessible.
  We then discuss why reproducibility also helps researchers to be more productive and produce higher-quality research.
  One example is that with reproducibility, researchers can plan studies and their analyses with great precision.
  If they publish such a plan, this is called "preregistration".
  We urge researchers to formulate their data analysis for a preregistration as computer code because computer code is more precise than describing it with words.
  They then should write all other parts of the preregistration with the same standards as a scientific article.
  We call this preregistration as code.
  Such a precise plan is beneficial for science because researchers generally trust results more if they are not explained after the fact.
...
Unfortunately, despite increasing commitment to open science practices, many projects can not yet be reproduced by other research teams [@obels2020].
...
This dynamic document closely resembles an actual journal article but with "fake" results and without discussion of results and serves as preregistration.
...
We suggest using Markdown for everything, starting from simple sketches of your ideas to your scientific manuscripts[@R-rticles], and presentations[@revealjs] or even your résumé [@vitae].
...
Researchers therefore have the opportunity to use the exact same software that was used by the original authors.
...
Such files are tiny (the [Dockerfile](https://github.com/aaronpeikert/repro-tutorial/blob/main/Dockerfile) for this project is `r round(as.numeric(fs::file_size("Dockerfile"))/1024^1, 2)`Kb big) and sufficiently describe how to recreate a given container image.
...
For archiving, we therefore recommend that a full image is always kept.
A more comprehensive overview over how to employ containarization in research projects is given by @wiebelsLeveragingContainersReproducible2021.
...
In the remainder, we will walk you through a the creation of a reproducible research project aided by the package `repro`.
...
Please install the package [`repro`](https://github.com/aaronpeikert)[@R-repro] so it can act as assistance systems while you follow the tutorial:
...
This type of markup for formatting, instead of the more visual approach of e.g. Microsoft Word ©, is compatible with many output formats and is more easily version controlled.
However, researchers might find it easier to activate the "Visual Editor" of R Studio (Ctrl + Shift + F4 or click on the icon that resembles drawing materials or a compass in the upper right corner of the R Markdown document) which features more graphical elements like a traditional word processor but still creates an R Markdown and has all of its benefits.
The Visual Editor has the additional benefit of promoting best practices, e.g. each sentence on a newline for better version control or automated citations, cross references and more.

Now that you are familiar with writing text we turn our attention to including code into the text.
...
R Markdown was extended to hundreds of document formats which are often highly configurable by their metadata.
...
In day-to-day work the simplest solution is to use the function `repro::automate()` which both infers a Makefile and a Dockerfile from all R Markdown  (all files with the ending `.Rmd`) and there respective repro metadata.
Every time you change the `repro`-metadata, change the output format, or add a new R Markdown, the Makefile and Dockerfile are potentially outdated, and you should run `repro::automate()` to keep them up to date.
...
However, users do not have to have repro installed to reproduce a project, in fact, they do not even need R installed. 
...
The last Dockerfile, `.repro/Dockerfile_packages`, should not be manually changed.
...
However, Docker requires single Dockerfile so `repro::automate()` simply combines the three Dockerfiles and saves the result into the main `Dockerfile` at the top level of the 

Users of `repro` can not only build arbitrarily complex software environments they can also implement complex file dependencies.
The standard `repro` metadata only makes sure that all dependencies are availible but does not allow you to specify a recipe for them in the metadata.
If you can formulate the creation of dependencies in terms of computational steps you should include these in the Makefile.
The Makefile that `repro` creates is only an template and you are free to change it.
However, you want to make sure to not remove the following two lines:
...
If you want to adept a target than simply provide an alternative target in the main `Makefile`.
Targets in the main `Makefile` take precedent.

The file `.repro/Makefile_Docker` does again contain a rather complicated template which you could but seldomly should alter.
...
To instead evaluate commends inside the Docker image you should wrap them in `$(RUN1) command $(RUN2)`, like in this example:
...
To summarise, `repro` provides untrained researchers with an easy to use solution for dependency tracking (in form of Make) and software management (using Docker).
Users with more advanced requirements can manually change all aspects while still profiting from can mix `repro`'s automations.
...
Both are valid implementation of Mann–Whitney–Wilcoxon test but the first assumes equal variance while the second applies Welch's correction and hence is robust even with unequal variances [@zimmermanRankTransformationsPower1993].
