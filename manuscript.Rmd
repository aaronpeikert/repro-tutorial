...
These so-called wrappers guide end-users in the use of reproducibility <!--CJ Can't say "standard tools" after just saying that these tools are not yet widely adopted -->tools, provide feedback about what the computer does, and suggest what the user should do in layman's terms.
...
These alternative tools<!-- CJ: Are they wrappers, or alternative tools--> are merely an assistance system, and as users get more and more comfortable, they can use the underlying tools directly to solve more complex problems.

This tutorial assumes that the user is working predominantly in R, with the help of RStudio.
It describes basic steps that we expect to apply to all users. Your specific situation might involve additional, more specialized steps. After completing the tutorial, you should be able to customize your workflow accordingly.<!--CJ: Cut/pasted from below; the purpose of the tutorial should be explained beforehand, not in the middle--> 
...
> install.packages('repro', repos = 'https://aaronpeikert.r-universe.dev')
Installing package into ‘C:/Git_Repositories/veni_forest/renv/library/R-4.1/x86_64-w64-mingw32’
(as ‘lib’ is unspecified)
Warning: dependencies ‘usethis’, ‘fs’, ‘gh’ are not available
trying URL 'https://aaronpeikert.r-universe.dev/src/contrib/repro_0.1.0.zip'
Error in download.file(url, destfile, method, mode = "wb", ...) : 
  cannot open URL 'https://aaronpeikert.r-universe.dev/src/contrib/repro_0.1.0.zip'
In addition: Warning message:
In download.file(url, destfile, method, mode = "wb", ...) :
  cannot open URL 'https://aaronpeikert.r-universe.dev/src/contrib/repro_0.1.0.zip': HTTP status was '404 Not Found'
Warning in download.packages(pkgs, destdir = tmpd, available = available,  :
  download of package ‘repro’ failed

But this one succeeds; it is what is suggested by R-universe:
> options(repos = c(
+     aaronpeikert = 'https://aaronpeikert.r-universe.dev',
...
repro::check_git()
  ...
  repro::check_docker()
```
<!-- CJ Be mindful of small spelling mistakes in the status messages, e.g., infos instead of info. Also: Not clear what different icons mean before the status messages. Also: Do you want to use informal language? E.g.: V Git is installed, don't worry! or: V Git is installed.--> 
These functions check whether specific dependencies are available on the users' system, and if not, explain what further action is needed to obtain it. <!-- CJ: This is so flipping important!!! We also need something like this for the summer school, to check if users have Mplus and if it can communicate with RStudio. I would emphasize this simple / valuable feature earlier, in the abstract and intro.-->
...
The messages emitted by `repro` and other packages we present <!-- CJ: This either needs more elaboration, or remove it entirely. It is currently confusing; what is the difference between "emitting" and "presenting"? I understand that you introduce some new functions, and import some existing ones. I would remove that distinction and just cite some of the packages you rely on.-->try to take the user by the hand to help them solve problems.
...
A shortcut to test whether you are ready to continue is:
...
Specifically, to synchronize your local repository with a remote repository on GitHub, issue the following command:
...
If you are not yet ready to share your project publicly<!--CJ: Note that you are conflating "online" with "public". People can push their repo to a private remote repository. Consider discussing this distinction first before showing the two commands.--> but you want to synchronize your local repository to a remote repository (for example, as a simple backup service), use:

```{r, eval=FALSE}
usethis::use_github(private = TRUE)
```

Once the connection with GitHub (or another remote repository, like GitLab or BitBucket) is set up, you can continue staging changes, committing them, and pushing your commits to the remote repository.
One convenience function that does all this in a single step is:

```{r}
worcs::git_update("Your commit message")
```

This is essentially a 'quick save' function that stages all changes to files not listed in `.gitIgnore`, commits them, and pushes the commit.
...
Using markup for formatting, instead of the visual approach used in, e.g., Microsoft Word, is compatible with many output formats and is easier to put under version control.
