...
This tutorial describes steps that we expect apply to all users, the messages from the packages fill the gap to your specific situation and extend the tutorial in this way.
...
To backup and share the files, you can publicly upload those changes to GitHub with:

```{r, eval=FALSE}
usethis::use_github()
```

Depending on your computer configuration this may ask to setup a secure connection to GitHub.

If you are not yet ready to share you project publicly use:
...
Three aspects are important to understand a dynamic document:
...
Code is separated by three backtics and the programming language in curly braces like here:
...
Now that you know how to write text and R code in an RMarkdown the only thing you need to know about is metadata, like the title or the output format.

Metadata are placed at the beginning of the document and are seperated by three dashes; the following example is a full markdown document where the header is in lines 1-6.
...
Most of the metadata fields are self explanatory (like the author field) but any package may use the metadata for their purposes.
...
The `repro` package uses the metadata, not to change the output, but to make the document more reproducible, simply by listing resources, i.e., scripts, data, and packages, your document depends on.
...
Besides, the Makefile and the Dockerfile repro generates a few more files in the `.repro` directory (which we will explain in detail later), all of which you should add and commit to Git.
...
Hence, it is necessary to understand what these commands do to satisfy more complicated requirements because then you need to understand how repro structures its interaction with Make and Docker.

Let us dissect the command for reproducing a repro project: `make docker && make -B DOCKER=TRUE`.
...
The `&&` combines two commands in one line.
The call `make` without any explicit target will build the Make target `all`, where the flag `-B` means that Make will consider all prerequisites as outdated and hence will rebuild everything.
repro constructs Make targets so that if you supply `DOCKER=TRUE` they are executed within the Docker image of the project.
