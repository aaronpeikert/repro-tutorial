...
<!-- lure the reader in -->
The research community has long accepted reproducibility as part of "good scientific practice" but has not benefited much from this self-imposed requirement.
...
You may be familiar with Microsoft Words' "Track Changes"-Feature and version control is similar in their basic idea but more potent in their features.
...
After fitting the model with strict measurement invariance, we free equality constraints first on loadings and then on intercepts based on the Lagrange Multiplier test.
...
Even when other researchers are excited to use your code or materials, they are usually legally inhibited from doing so.
...
Such files are tiny (the Dockerfile for this project is `r round(as.numeric(fs::file_size("Dockerfile"))/1024^1, 2)`Kb big), but they rely on the assumption that all software repositories are available and contain the correct software version.
...
However, even when your project relies solely on R, external dependencies still change the outcome of computations, like the operating system, the compilers used, the BLAS system in use and so forth.
...
One usecase in which Docker sometimes can't be used is in high performance computing (HPC).
...
However, synthetic data provides an opportunity for researcher to test if a project is in principle reproducible, even though they have no access to the data. 
...
```{r, include=FALSE}
knitr::write_bib(c(.packages(), "bookdown", "lavaan", "knitr", "targets", "renv"), "packages.bib")
```
