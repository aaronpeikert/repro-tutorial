...
While you may be tempted to consider reproducibility only when you are close to completing a project, we believe that upfront investment pays off in your increased productivity.
...
Monte Carlo simulations are a crucial tool to investigate properties of a statistical model and they are routinely employed to develop or evaluate statistical models.
For example, @maccallum1999sample used Monte Carlo simulations to investigate optimal sample size for exploratory factor analysis, or @curran1996robustness studied the robustness of the $\chi^2$ test statistic under non-normal data. In Bayesian data analysis, prior predictive checks generate data from a model according to the prior distribution in order to asses whether a prior is appropriate [@gabry2019visualization].
...
A Monte Carlo simulation affords you an ideal planning opportunity.
We realize that this comes at an expensive but believe the reduction in the risk that the data is not sufficient to answer the research questions offsets the required effort.
This advantage is especially pronounced when you plan to preregister your study.
In fact, we argue that a Monte Carlo simulation can combine the analysis, sample size planning and preregistration (see [Preregistration as Code].
...
We propose the following procedure for analyzing the data and dealing with possible violations of strong measurement invariance. 
After fitting the model with strong measurement invariance [@flake2020], we automatically free equality constraints, first on loadings, and then on intercepts based on the Lagrange Multiplier test.
...
Without getting into too much detail of the meta-scientific arguments for preregistration, we want to note two advantages [@nosekPreregistrationHardWorthwhile2019].
...
This approach allows for more flexibility than a traditional template based preregistration but is still well defined.
...
One way to ensure that your analysis to works with real data is to simulate data.
...
A reader of a reproducible research project should be able to easily derive how it can be reproduced (ideally by a statement in your [README]).
To do that, it is often crucial to understand how different project components relate to each other and depend on each other.
For example, you cannot run an analysis without data, or you cannot render a statistical report without having the results from the statistical analysis that is reported.
Dependency tracking means that you capture and map the interrelations between all components of your analysis.
The "components" of an analysis are typically all involved files including raw data files, intermediate files (such as a preprocessed data file), and final results (such as a rendered manuscript in PDF format).
With larger projects, there is typically a higher degree of modularization because code fragments are written for re-use in other projects (or are already re-used). Also, more complicated empirical research projects may have different hypotheses that are answered with different data sets and different statistical models, such that it often makes sense to split the different analyses into different files. Reproducibility then demands the careful documentation and mapping of all necessary steps in the correct order.

Make is a build automation tool for software projects that typically are challenged by maintaing and resolving a variety of dependencies in the correct order when compiling a piece of software-
We found _Make_ to be particularly useful for our purposes for two reasons.
First, it is language-agnostic, therefore enabling us to map steps not only taken within R but also across different programming languages or other external programs.
...
That is, Make determines automatically which branches in the dependency graph needs updating when files have changed and does not necessarily recompute or rebuild everything.
