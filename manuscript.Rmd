...
The end goal of this workflow is a project that can be completely, automatically, and unambiguously reproduced.<!--CJ From tutorial-->
This means that any manual steps, such as copy and pasting results, manual modification of data (e.g., manually removing observations or manually recoding variables),
are replaced with reproducible code. 
The result of this workflow will be a portable research project containing all digital research artefacts,
in which all steps required to produce the (published) manuscript from the raw (and ideally, open) data are well-documented
and automatically reproducible with a single command.<!--CJ From tutorial-->
...
The first part of this tutorial outlines threats to reproducibility and how the here presented reproducibility standard addresses these threats.
...
**Docker** is a popular protocol for containerization.
...
This is important because some of its functionality may impact the workings of higher-order software, such as calls to random number generators, linear algebra libraries and such within R.<!-- CJ From tutorial-->

Note that the software environment of the Docker image is completely separate from software installed on your computer.<!-- CJ This paragraph is also relevant for users that are not advanced!!! Moved it here, from the tutorial-->
This separation is excellent for reproducibility, but take some getting used to.
...
In the remainder, we will walk you through the creation of a reproducible research project with the package `repro`.<!-- CJ From tutorial-->
...
We assume that you have installed R and RStudio already but the tutorial will guide you in detail through the installation of other necessary software with the help of the R package [`repro`](https://github.com/aaronpeikert)[@R-repro].
...
abstract: A tutorial about doing the same thing more than once.<!-- CJ I actually really like this title, and would consider it for the main manuscript. E.g., "Reprodicible research: A tutorial about.."-->
...
title: A Hitchhiker's Guide to Reproducible Research in R
author: Aaron Peikert, Caspar J. Van Lissa and Andreas M. Brandmaier
abstract: A paper about recalculating the same thing more than once.<!-- CJ Is it correct that this is different from the example above?-->
output: html_document
repro:
  scripts:
    - R/load.R
...
This information makes explicitly declares the project's dependencies on files and R packages.<!-- CJ How about version numbers and sources? Would it make sense to declare a dependency on renv and include a .renv file?-->
...
<!-- CJ This paragraph needs to be elaborated further-->

# Advanced Features

This section is only for advanced users who want to overcome some limitations of `repro`.<!-- CJ Consider sticking this in a vignette, which would allow it to be updated more regularly-->
...
Whenever `repro::automate()` is called, repro gathers all R packages from all `.Rmd` files and figures out whether they should be installed from CRAN or GitHub fixed to the date specified in `Dockerfile_base`.<!-- CJ Suggestion: Make a directory called "dockerfiles", and name the files "image", "external_dependencies", and "rpackages"-->

Finally, there is one manually edited Dockerfile: `.repro/Dockerfile_manual`. <!-- CJ I would expect this file to contain a "manual" based on the title. Consider if there is a better name, eg: .repro/external_dependencies-->
...
<!--Note that the software environment of the Docker image is completely separate from software installed on your computer. CJ This paragraph is also relevant for users that are not advanced!!!
This separation is excellent for reproducibility, but take some getting used to.
For example, it is important to realize that software available on your local computer will *not* be accessible within the confines of the Docker image.
Each dependency that you want to use within the Docker image must be explicitly added as a dependency.-->
Using Docker mayrequire you to install software on an operating system that may not be familiar to you.
The images supplied by [@boettigerIntroductionRockerDocker2017], for example, are based on the Ubuntu operating system.
The most convenient way to install software on Ubuntu is through its package manager `apt`.
The following snipped installs Python.<!--CJ You have not explained why one might want to install python. I think this is too specific for the general paper; put in a vignette and make a separate vignette for "Adding external dependencies",addressing python and others-->
...
<!--
Docker eventually requires a single Dockerfile to run, so `repro::automate()` simply concatenates the three Dockerfiles and saves the result into the main `Dockerfile` at the top level of the R project.CJ Too much technical detail about the implementation. This section promised to help advanced users enhance the functionality of repro - but what functionality are they gaining by knowing that repro concatenates these files? -->
...
The file `.repro/Makefile_Docker` does again contain a rather complicated template which you could but usually should not modify.<!-- CJ Under what conditions could you modify it? -->
...
If you write a computational recipe<--CJ this term has not been defined--> for a target it will be by default evaluated using the locally installed software.
...
<!-- CJ I would only leave one paragraph for advanced users, stating that they can manually change the Dockerfiles and makefiles following the vignette on the repro website. Skip the details about implementation in this paper!!! That's not what a tutorial is for-->
...
Preregistration increases the credibility of empirical results by separating theory-driven planned analyses from unplanned exploratory analyses.
Preregistration seeks to reduce "researcher degrees of freedom" [@reference] in the execution of a planned study by limiting the number of decisions to be made after data has been collected, which could influence the results.
However, there are several shortcomings to preregistration.
One shortcoming is that written study plans are often multi-interpretable.
Even when several researchers set out to investigate the same hypothesis using the same data, their approaches vary considerably [@silberzahnManyAnalystsOne2018].
...
However, templates cannot remove researchers degrees of freedoms completely,
because it is impossible to verbally describe every detail of an analysis for all but the simplest analysis.
...
The workflow described in this paper facilitates a rigorous and innovative solution to this problem:
...
Moreover, it removes the necessity to write two separate documents: A preregistration, and a manuscript.
...
This includes the Introduction, Methods, and Results sections.
...
This tutorial integrates the preregistration of code into the research workflow
by encouraging researchers to preregister the whole manuscript as a reproducible dynamic document.
...
First, in PAC, the preregistration is simply a first draft of the final manuscript,
thus sparing authors from writing (and editors and reviewers from evaluating) two separate documents.
...
Researchers can, for example, formulate conditions (`if() {} else {}`) under which they prefer one analysis type over the other.
...
Fifth, a PAC is more comprehensive by design because its completeness can be empirically checked with simulated data.
...
Sixth, deviations from the preregistration are clearly documented, because they are reflected in changes to the code which are managed and tracked with version control.
...
When writing a PAC, researchers ought to strive to meet three goals:
...
Describing the preregistration in the form of a journal article requires authors to follow the same standards that also ensure that research articles are comprehensive enough to replicate the described study.
...
It can be fruitful to consult preregistration templates, because of there strong focus on formulating a comprehensive methods section [@ref provide specific good examples].
...
<!-- CJ: The text below is about reporting standards; it's useful, but not directly linked to the text above-->
...
The present example illustrates this ambiguity, 
because the functions `wilcox.test` and the combinations of `rank` and `t.test` are both valid implementation of the Mann–Whitney–Wilcoxon test.
However, whereas the former assumes equal variance whereas the latter is robust even with unequal variances across groups [@zimmermanRankTransformationsPower1993].
Mentioning every such detail in a written preregistration plan is almost impossible.
Still, these details impact the results and provide unwanted researcher degrees-of-freedom.
...
At the time of preregistration, researchers should remain insulated from real data.
...
res %>%
  ...
  NULL
```
<!-- CJ This section also needs a concluding paragraph to wrap it up and present a take-home message, before moving on to Discussion-->

# Discussion

## Summary

This paper demonstrated how the R package `repro` supports researchers in creating reproducible repositories and reproducible manuscripts.
...
To benefit from automatic and convenient reproduction researcher must install Git, Make, and Docker.
...
This process is technically possible and would guarantee reproducibility for decades<!--CJ It's hard to know this for sure--> without any software dependency.
...
Hypothetically, even if Docker were unavaible, the Dockerfile still serves as unambiguous documentation of how the original system was set up and may help future users to create a software environment that closely resembles the original.

# Limitations

We realize that PACs are challenging.<!--CJ Why a separate limitations section for PACs??? I don't think this is a very strong section, and if PACs are just an example of innovations afforded by repro, then it does not warrant a separate Limitations section. Discuss only the MOST IMPORTANT points below in the PAC section instead-->
...
The rigidity of PAC is only than to much of a good thing, if one holds the view that researchers can for no reason deviate from their preregistration.
...
We call the property of code that can run on different inputs from a similar context and produce valid outputs "reusability".<!--CJ This property is relevant for the whole workflow, so it needs to go into the general discussion-->
