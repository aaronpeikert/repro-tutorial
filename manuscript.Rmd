...
Of course, one has to make many educated guesses to simulate data that closely matches your expectation.
These guesses strongly depend on the study you are planning, and there cannot be a one fits all solution.
However, the analytic strategy often gives you a hint about how to simulate data.
In the Machiavellism example, the analytic strategy is to use structural equation modelling (SEM).
...
The model parameters should mimic the Machiavellism subscale of the short dark triad (SD3).
...
Therefore, we will assume that the loadings are 30% lower than reported and that the standardized mean difference is only 0.1.
These assumption can be simply translated into [`lavaan` [@lavaan]](https://lavaan.ugent.be)[^lavaan] model syntax:
...
It is probably more realistic to violate this assumption and plan our analysis accordingly.
Therefore, we simulate the data so that some of the items parameter differ among groups and then alter the analysis to assume partial measurement invariance.
...
Hence, we continue simulating continues data.
However, adding a little skewness to the items is easy and helps make the simulation more realistic.
...
Note however that this function is again composed of several functions, which you may inspect on `r link("https://github.com/aaronpeikert/repro-tutorial/blob/master/R/simulation.R", "GitHub")`.

With its help we can simulate 500 cases with a mean difference between groups of 0.1, where the intercepts of three random items vary by .3 and three loadings vary by 30% and five random items have a skew.

```{r}
simulated <- generate(n = 500,
                      diff = 0.1,
                      vary_intercepts = rep(0.3, 3),
                      vary_loadings = rep(0.3, 3),
                      skewness = rep(5, -0.5))
```

Note, that this certainly results in different numbers each time we run it.
To consistently get the same results it is good practice to use a seed.
The function `seeded_generate()` will always generate the same pseudo-random numbers, when given the same seed.

```{r}
seeded_generate <-  function(seed, ...)withr::with_seed(seed, generate(...))
simulated <- seeded_generate(
  seed = 1234,
  n = 100000,
  diff = 0.1,
  vary_intercepts = rep(0.3, 3),
...
```{r}
lavaan::summary(analyze(simulated))
```
...
While you already know how to write dynamic documents with [Markdown] and [R Markdown], and track changes with [Git] and [GitHub], two components to archive longterm reproducibility are still missing.

## Internal Dependencies

<!--I am not sure this is the right place, should we introduce this earlier? -->

First dependency management, which means that you capture the interrelations between the components of you analysis.
You cannot run an analysis without data, you cannot render a dynamic document without an analysis, and so forth.
Understanding these relations is often key to reproducing a project.
...
However, while it is easy to reproduce this approach quickly gets unwieldy.
