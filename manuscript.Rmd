...
It shows how to create a heading, a word in bold font, and a list of several items in Markdown:<!-- CJ Consider suggesting a specific template, like papaja, that has such an example by default - so users only need to edit the existing template. BTW - is there a check_papaja()? I want one!-->
...
`repro` uses this information to infer a Makefile<!--CJ Has not been introduced yet--> for the dependencies on other files and a Dockerfile<!--CJ Has not been introduced yet--> that includes all required packages.
...
repro::reproduce()
```
<!-- CJ: You didn't say anything about using the terminal.... or will that be explained by messages printed by the code above? Either way, I suggest making the fully automatic version the default instead of making users interact with system().-->
...
Users do not have to have repro installed to reproduce a project, in fact, they do not even need to have R installed because the entire project can be rebuilt inside of a container that has R installed. <!-- CJ This could go into the "reproducing a project" section. BTW, it is important to be clear about what *does* need to be installed. For example, I don't find it objectionable to have to install R. But do users need to install docker?-->
The two Make commands `make docker` and `make -B DOCKER=TRUE` together with an installation of Make and Docker are sufficient.<!--CJ This really belongs in a "reproducing a project" section"-->
...
CJ: I completely agree with this, too.-->

# Advanced Features

Some projects, however, are not as easy to make reproducible.<!--CJ Many might not find the above 'easy'. Consider focusing on 'custom workflows' instead of calling the above easy.-->
As explained above, `repro` is merely a simplified interface that guarantees a high level of reproducibility for simple projects.<!--CJ This is confusing. How can an interface guarantee reproducibility? Does repro guarantee reproducibility? If so, how? Why is this only suitable for simple projects? What makes a project simple?-->
These simplifications<!--CJ Which simplifications?--> are possible because of two restrictions.
Users who ask themselves either "How can I install software dependencies outside of R in the Docker image?" or "How can I express complex dependencies between files, e.g. raw data turned into preprocessed data required by analysis?"<!--CJ Only if the preprocessing happens outside of R, right? Isn't that covered by the previous point? How about instead: Users that need to work on a access restricted system, that's a very common hurdle I hear--> need to be aware of these restriction and require a deeper understanding of the inner workings of `repro`.
