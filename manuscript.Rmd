...
Sharing these digital artefacts <!-- consider using the term 'research output' instead, as digital artefacts might be unnecessarily technical-->increases scientific efficiency by enabling researchers to learn from each other, reuse materials, or review published research results, but only if these artifacts are reproducible<!--Does this last statement apply to data and materials? Reproducible materials?-->.
<!-- Suggest adding sentence: The technology is now available to make all artefacts related to a research project available in such a way that the project can be reproduced with minimal effort. --> 
...
This is because making a research project reproducible <!-- how does a research project relate to data/materials/code/artefacts?--> comes with a variety of challenges, and there is a dire need for reproducibility standards.

This paper has two aims.
First, we want to introduce the reader to a workflow that ensures reproducibility [see @Peikert2019 for details]. 
<!--Second? This is really a different point than introducing the workflow-->We present four common threats to reproducibility and explain how we can address these with a combination of four solutions from the software engineering community.
<!-- Third? -->Specifically, we demonstrate how the R package `repro` supports researchers in implementing this solution.
<!-- Not sure if this belongs in the present paper. If it does, then I wouldn't introduce it as a separate goal. Instead, I would say something like: The present paper introduces the reader to a workflow that ensures reproducibility by building on several technical solutions originating in the software engineering community. We show how these solutions address several common threats to reproducibility, and enable novel solutions further enhancing the open science workflow, such as 'preregistration as code'.  -->The second objective is to present an alternative preregistration scheme that builds upon a reproducible workflow, the so-called preregistration as code (PAC), where the researchers write the complete analysis as code and the bulk of the manuscript, including the sections Introduction, Methods, and Results---initially based on simulated data, before they gather data.
...
While the willingness to share materials is required for reproducing a project, it is not sufficient.<!-- This sounds like the start of a new paragraph-->
...
After introducing the conceptual building blocks, this tutorial guides the reader through each step of an open research project, <!--What does this mean? Isn't the whole paper about reproducibility?-->emphasising guaranteeing reproducibility from the very first step on.
