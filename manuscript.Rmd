...
We demonstrated how the R package 'repro' supports researchers in creating reproducible repositories and reproducible manuscripts. These are important building blocks for transparent and cumulative science
because they enable others to reproduce statistical and computational results and reports later in time and on different computers. This is made possible because the repro approach fully documents all software dependencies from the exact version of specific R packages down to the entire operating system layer. Using a virtualization approach, the entire software environment can be recreated on a different computer without interfering with locally installed software. 
Building upon reproducible manuscripts, we suggest to use reproducible PAC as a means to create and publish unambiguous pre-registrations in the form of code. Ideally, a PAC contains all analysis code and a reproducible manuscript that already renders the entire reporting of statistical and computational results. These results are based on simulated data, such that there is no danger of peeking at the real data while writing the scripts. This way, all researcher's degrees of freedom are eliminated. Once real data is gathered, the reproducible manuscript is (re-)created with the real data. All deviations from the pre-registration can be documented with version control systems that produce accurate documentations of the differences in the analysis code.

## Limitations of reproducibility

<!--AB: please add what possible problems are there that endanger reproducibility; what are the assumptions under which this all works... -->
Reproducible repositories created with the proposed workflow will work as long as operating systems with support for Make, Docker, and git are available.
If development of git should be stopped one day, we believe that there will be other version control systems that will offer similar features and that can be substituted for git.
If Make is not available on a future system, the Makefile itself is written in a way that one can manually trace the dependencies and execute commands in the right order.
If Docker is not available on a future system, reproducing the comoutational environment will be more difficult, but the Dockerfile will still serve as unambiguous documentation of how the original system was set up and may help future users to create a software environment as close as possible to this environment.
<!--AB: please finalize; say something to the effect of Dockerfiles vs full containers (again)?-->
<!--AB: are there any specific limitations of what the package can/cannot do?-->

## Limitations of PAC

PAC comes with challenges.
In contrast to a reproducible script that needs to work on a single, given dataset, a PAC must work across different data sets that are yet unknown at the time the script is created.
...
The reproducible workflow we present here is heavily automated and hence promotes reusability; however, there is an increased demand on the researchers' side to write and maintain code that is flexible enough to work with different inputs. 
...
Another challenge are analyses that do not have linear execution but branch out. If such conditional analysis steps exist (such as those we demonstrated before with the test of mean differences), scholars using PAC need to write code that may be never executed.
Typical examples of conditional analysis steps could be an exploratory factor analysis that is only executed if the theory-driven model does not fit well, or a group comparison is only executed if measurement invariance in a latent factor model is established.
Writing code that may be never executed puts additional burden on the researcher.
However, we hope that with the principle of reusability in mind, open and reproduce PAC elements may be reused by others and in turn save time to future developers.
...
## Limitations of the test for mean differences
...
<!--AB: TODO for AP: two/three lines of what the future of the package, of such standards, and prereigstration is. Maybe say something that preregistration is not always possible or wanted but that reproducibility is almost always a good idea?-->
