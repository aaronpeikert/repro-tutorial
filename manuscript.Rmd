...
Projects that combine Make and Docker and adhere to the standard proposed by @Peikert2019 can be reproduced with:
...
Because you probably want to reproduce your own project each time something changes, you will often want to reproduce everything.
...
If you have written a [Preregistration as Code] you can add the real data file to the project and reproduce the entire project with this single command.
...
A prerequisite to reproducibility is the permanent availability of both code and data.
We use containers to guarantee that all code components are available and work as expected.
Ideally, all data are also stored in the repository [for a more comprehensive argument for publishing data see @vanlissa2020worcs], but this is not always feasible.
