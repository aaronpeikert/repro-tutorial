...
res_interval <- res %>%
  ...
  group_by(alpha) %>% 
  arrange(n_obs) %>% 
  mutate(across(c(width, upper, lower), list(smooth = ~slide_dbl(.x, mean, na.rm = TRUE, .before = 2, .after = 0, .complete = FALSE)))) %>% 
  ungroup()
```
...
plot_interval %+% aes(ymin = lower_smooth, ymax = upper_smooth)
```
